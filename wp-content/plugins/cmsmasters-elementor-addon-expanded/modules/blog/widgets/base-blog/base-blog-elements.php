...
abstract class Base_Blog_Elements extends Base_Blog {
	...
	protected function register_header_section_controls() {
		...
		$this->add_control(
			...
			array(
				...
				'placeholder' => esc_html__( 'My Posts', 'cmsmasters-elementor' ),
				'condition' => array(
					'header_show!' => '',
					'header_title_show!' => '',
				),
	...
	protected function render_header() {
		...
		if (
			! $settings['header_show'] ||
			(
				$settings['header_show'] &&
				! $settings['header_filter_show']
			)
		) {
			return;
		}

		echo '<div class="cmsmasters-blog-header' . ( 'multiple-rows' === $settings['filter_view'] ? " cmsmasters-blog-filter-nav-multiple-rows-{$settings['filter_multiple_rows_layout']}" : '' ) . '">
	...
	protected function render_filter() {
		...
		echo '<div class="cmsmasters-blog-filter">
			<ul class="cmsmasters-blog-filter-nav cmsmasters-blog-filter-nav-primary">
				...
				foreach ( $term_ids as $term_id ) {
					...
					if ( $posts_query->have_posts() ) {
						...
						echo '<li>' .
							'<a ' . ElementorUtils::render_html_attributes( $term_link_attrs ) . '>';
			...
			if ( 'multiple-rows' !== $settings['filter_view'] ) {
				...
				<a href="#" class="cmsmasters-blog-filter-nav-secondary-trigger">';
					Utils::render_icon( array(
