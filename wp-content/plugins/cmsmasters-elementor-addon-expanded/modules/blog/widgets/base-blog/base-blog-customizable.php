...
abstract class Base_Blog_Customizable extends Base_Blog_Elements {
	...
	protected function init( $data ) {
		...
		$this->template_id = (int) $this->get_settings_for_display( 'blog_template_id' );
	}
	...
	public function get_template_ids() {
		...
		if ( ! Utils::is_publish( $this->template_id ) ) {
			return array();
		}
	...
	protected function get_controls_to_hide_for_template() {
		...
		foreach ( $sections_ids as $section_name ) {
			$section_controls = array_filter( $this->get_section_controls( $section_name ), function( $control ) {
				return ! isset( $control['groupPrefix'] );
			} );

			array_push( $controls_ids, $section_name );

			$controls_ids = array_merge( $controls_ids, array_keys( $section_controls ) );
		}
	...
	protected function render_post_inner() {
		if ( $this->is_setting_as_default( 'blog_layout' ) ) {
			...
			echo $addon->frontend->get_widget_template( $this->template_id, false, true );
		}
	}
	...
	protected function render_post_thumbnail() {
		...
		echo '<div class="cmsmasters-blog__post-thumbnail">' .
			'<a href="' . esc_attr( get_permalink() ) . '" class="cmsmasters-blog__post-thumbnail__inner">' .
	...
	public function render_read_more() {
		...
		echo '<a class="cmsmasters-blog__post__read_more cmsmasters-theme-button" href="' . get_permalink() . '">';
