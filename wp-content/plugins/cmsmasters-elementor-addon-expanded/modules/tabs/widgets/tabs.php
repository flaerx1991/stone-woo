...
class Tabs extends Base_Widget {
	...
	protected function register_controls() {
		...
		if ( '' !== $this->get_page_template_options( 'section' ) ) {
			$repeater->add_control(
				'saved_section',
				array(
		...
		foreach ( array(
			...
			$this->start_controls_tab(
				...
				$this->add_group_control(
					...
					array(
						'name' => "tab_list_item_bg_group_{$main_key}",
						'exclude' => array( 'color' ),
						'selector' => $background_selector,
					)
				);

				$this->start_injection( array( 'of' => "tab_list_item_bg_group_{$main_key}_background" ) );
	...
	public function render() {
		...
		echo '<div class="cmsmasters-tabs" role="tablist">';
		...
		echo '<div class="cmsmasters-tabs-list-wrapper">
			...
			foreach ( $tabs as $index => $item ) {
				...
				$this->add_render_attribute( $tab_title_setting_key, array(
					'id' => 'cmsmasters-tabs-list-item_' . $id_int . $tab_count,
					'class' => array( 'cmsmasters-tabs-list-item', $animation_class ),
					'data-tab' => $tab_count,
				) );
				...
				if ( '' !== $tab_custom_id ) {
					...
					$this->add_render_attribute( $tab_title_setting_key, array( 'tab_custom_id' => $tab_custom_id ) );
				}

				$this->add_render_attribute( $tab_title_setting_key . '_text', 'class', array(
					'cmsmasters-tab-title__text',
					( 'Tab' === $item['tab_title'] ) ? 'default' : '',
				) );
				...
				if ( '' !== $item['tab_title'] || '' !== $item['tab_subtitle'] ) {
					echo '<div class="cmsmasters-tab-title__text-wrap">';
						echo ( '' !== $item['tab_title'] ) ? '<' . $title_tag . $this->get_render_attribute_string( $tab_title_setting_key . '_text' ) . '>' . $item['tab_title'] . '</' . $title_tag . '>' : '';
						echo ( '' !== $item['tab_subtitle'] ) ? '<span class="cmsmasters-tab-subtitle-text">' . $item['tab_subtitle'] . '</span>' : '';
					echo '</div>';
				}
	...
	private function print_icon( $item ) {
		...
		if ( '' !== $item['tab_icon']['value'] ) {
			$html = '<span class="cmsmasters-tab-icon' . esc_attr( $tab_icon . $tab_list_item_style_icon_position ) . '"></span>';
		}
	...
	private function print_tabs_content( $tabs ) {
		...
		foreach ( $tabs as $index => $item ) {
			...
			$this->add_render_attribute( $tab_content_setting_key, array(
				'id' => 'cmsmasters-tab-content_' . $id_int . $tab_count,
				'class' => array( 'cmsmasters-tab' ),
				'data-tab' => $tab_count,
			) );
	...
	private function print_accordion_start( $id_int, $index, $item ) {
		...
		echo '<div class="cmsmasters-accordion-item-wrap">';
			...
			$this->add_render_attribute( $accordion_title_setting_key, array(
				'id' => 'cmsmasters-accordion-item-' . $id_int . $tab_count,
				'class' => array( 'cmsmasters-tabs-list-item', 'cmsmasters-accordion-item', $animation_class ),
				'data-tab' => $tab_count,
			) );
		...
		if ( '' !== $item['tab_title'] || '' !== $item['tab_subtitle'] ) {
			echo '<div class="cmsmasters-tab-title__text-wrap">';
				echo ( '' !== $item['tab_title'] ) ? '<' . $title_tag . $this->get_render_attribute_string( $accordion_title_setting_key . '_text' ) . '>' . $item['tab_title'] . '</' . $title_tag . '>' : '';
				echo ( '' !== $item['tab_subtitle'] ) ? '<span class="cmsmasters-tab-subtitle-text">' . $item['tab_subtitle'] . '</span>' : '';
			echo '</div>';
		}
	...
	public function print_accordion_icon( $settings, $is_woo ) {
		...
		if ( '' !== $settings['accordion_icon']['value'] ) {
			$accordion_icon_closed = '<span class="cmsmasters-tab-icon cmsmasters-accordion-closed' . esc_attr( $accordion_icon . $accordion_item_style_icon_position ) . '"></span>';
			$accordion_icon_open = '<span class="cmsmasters-tab-icon cmsmasters-accordion-opened' . esc_attr( $accordion_icon_active . $accordion_item_style_icon_position ) . '"></span>';

			if ( 'svg' === $settings['accordion_icon']['library'] ) {
				$accordion_icon_closed = '';

				echo '<span class="cmsmasters-tab-icon svg cmsmasters-accordion-closed">';
					Icons_Manager::render_icon( $settings['accordion_icon'] );
				echo '</span>';
			}
			...
			if ( 'svg' === $settings['accordion_icon_active']['library'] ) {
				...
				echo '<span class="cmsmasters-tab-icon svg cmsmasters-accordion-opened">';
					Icons_Manager::render_icon( $settings['accordion_icon_active'] );
				echo '</span>';
			}
		}

		echo $accordion_icon_closed . $accordion_icon_open;
