...
class Hotspot extends Base_Widget {
	...
	protected function register_controls() {
		...
		foreach ( $state as $key => $label ) {
			...
			$this->add_group_control(
				...
				array(
					...
					'selector' => "{{WRAPPER}} .elementor-widget-container > img{$state}",
				)
			);
		...
		foreach ( $colors as $key => $label ) {
			...
			$this->add_group_control(
				...
				array(
					'name' => "hotspot_tooltip_button_bg_{$key}",
					'exclude' => array( 'color' ),
					'selector' => $selector . $element,
					'condition' => array(
						'hotspot_tooltip_items_type' => 'button',
					),
				)
			);

			$this->start_injection( array( 'of' => "hotspot_tooltip_button_bg_{$key}_background" ) );
		...
		foreach ( $settings['hotspot'] as $key => $hotspot ) {
			...
			$this->add_render_attribute(
				$hotspot_repeater_setting_key, array(
					'class' => array( 
						...
						( 'click' === $settings['tooltip_trigger'] && $is_hotspot_link ) ? "{$this->hotspot_selector}__no-tooltip" : '',
					),
				)
			);
			...
			$this->add_render_attribute(
				$tooltip_repeater_setting_key, array(
					'class' => array(
						...
						( $show_tooltip ) ? "{$this->hotspot_selector}__show-tooltip" : '',
						( ! $is_tooltip_direction_animation ) ? "{$this->hotspot_selector}__tooltip-position" : '',
			...
			echo "<{$tag} {$this->get_render_attribute_string( $hotspot_repeater_setting_key )}>
				<div {$this->get_render_attribute_string( $trigger_repeater_setting_key )}>";
					...
					if ( $is_circle ) {
						echo "<div class='{$this->hotspot_selector}__outer-circle'></div>
						<div class='{$this->hotspot_selector}__inner-circle'></div>";
					} else {

						if ( $hotspot['hotspot_icon']['value'] || $hotspot['hotspot_icon_active']['value'] ) {
							echo "<div class='{$this->hotspot_selector}__icon-wrapper'>";

								if ( $hotspot['hotspot_icon']['value'] ) {
									echo "<div class='{$this->hotspot_selector}__icon'>";
										Icons_Manager::render_icon( $hotspot['hotspot_icon'] );
									echo "</div>";
		
									if ( $hotspot['hotspot_icon_active']['value'] ) {
										echo "<div class='{$this->hotspot_selector}__icon {$this->hotspot_selector}__icon-active'>";
											Icons_Manager::render_icon( $hotspot['hotspot_icon_active'] );
										echo "</div>";
									}
								}
						...
						if ( $hotspot['hotspot_label'] ) {
							echo "<div class='{$this->hotspot_selector}__label'>
				...
				if ( $hotspot['hotspot_tooltip_content'] && ! ( 'click' === $settings['tooltip_trigger'] && $is_hotspot_link ) ) {
					if ( $is_tooltip_direction_animation ) {
						echo "<div {$this->get_render_attribute_string( $direction_mask_repeater_setting_key )}>";
					}
	...
	protected function render_button( $hotspot ) {
		if (  $hotspot['hotspot_button_switcher'] ) {
			ob_start();
			...
			$button_text = ( '' === $hotspot['hotspot_button_label'] ) ? __( 'Click Here', 'cmsmasters-elementor' ) : $hotspot['hotspot_button_label'];
		...
		_.each( settings.hotspot, ( hotspot, index ) => {
			...
			view.addRenderAttribute( tooltipRepeaterSettingKey, {
				'class': [
					...
					( showTooltip ) ? 'elementor-widget-cmsmasters-hotspot__show-tooltip' : '',
					( !isTooltipDirectionAnimation ) ? 'elementor-widget-cmsmasters-hotspot__tooltip-position' : '',
