...
class Wbw_Product_Filter extends Base_Widget {
	...
	protected function register_controls() {
		...
		foreach ( $colors as $key => $label ) {
			...
			$this->add_group_control(
				...
				array(
					'name' => "wbw_button_bg_{$key}",
					'exclude' => array( 'color' ),
					'selector' => "#cmsmasters_body {{WRAPPER}} .cmsmasters-wbw-product-filter.cmsmasters-theme-style button{$state}{$element}",
				)
			);

			$this->start_injection( array( 'of' => "wbw_button_bg_{$key}_background" ) );
	...
	protected function render() {
		...
		echo "<div {$this->get_render_attribute_string( 'wbw_pf' )}>";
