...
class Cart extends Base_Widget {
	...
	public function get_counter() {
		...
		if ( 'none' !== $settings['counter'] ) {
			$out .= Utils::get_ob_html( array( __CLASS__, 'get_counter_inner' ) );
		}
	...
	public function get_button_text() {
		...
		if ( isset( $settings['button_text'] ) ) {
			...
			if ( '' === $settings['button_text'] ) {
				$button_text = __( 'Cart', 'cmsmasters-elementor' );
			}
	...
	public function get_subtotal() {
		...
		if ( 'true' === $settings['subtotal'] ) {
			$out .= '<span class="elementor-widget-cmsmasters-woo-cart__button-subtotal">' .
				Utils::get_ob_html( array( __CLASS__, 'get_subtotal_inner' ) ) .
			'</span>';
		}
	...
	public function get_close() {
		...
		$this->add_render_attribute( 'cart-close', 'class', 'elementor-widget-cmsmasters-woo-cart__cart-close' );

		if ( isset( $settings['close_position'] ) ) {
			$this->add_render_attribute( 'cart-close', 'class', 'cmsmasters-position-' . $settings['close_position'] );
		}

		if ( isset( $settings['close_alignment'] ) ) {
			$this->add_render_attribute( 'cart-close', 'class', 'cmsmasters-align-' . $settings['close_alignment'] );
		}

		if ( isset( $settings['close_content'] ) && 'icon-text' === $settings['close_content'] ) {
			$this->add_render_attribute( 'cart-close', 'class', array(
				'cmsmasters-icon-text',
				'cmsmasters-icon-' . $settings['icon_close_position'],
			) );
		}

		if ( isset( $settings['close_view'] ) ) {
			$this->add_render_attribute( 'cart-close', 'class', 'cmsmasters-view-' . $settings['close_view'] );
		}

		if ( isset( $settings['close_shape'] ) ) {
			$this->add_render_attribute( 'cart-close', 'class', 'cmsmasters-shape-' . $settings['close_shape'] );
		}

		$out = '<div ' . $this->get_render_attribute_string( 'cart-close' ) . '>';

		if ( 'text' !== $settings['close_content'] ) {
			$out .= Utils::get_render_icon( $settings['close_icon'], $attributes = array( 'aria-hidden' => 'true' ) );
		}

		if ( 'icon' !== $settings['close_content'] ) {
			$out .= '<span class="elementor-widget-cmsmasters-woo-cart__close-label">' . $settings['close_label'] . '</span>';
		}
	...
	protected function render() {
		...
		if ( 'popup' === $settings['cart_type'] ) {
			$this->add_render_attribute( 'cart-container', 'class', 'cmsmasters-woo-cart-position-' . $settings['button_alignment'] );
		}

		$this->add_render_attribute( 'button-content', 'class', array(
			'elementor-widget-cmsmasters-woo-cart__button-content',
			'cmsmasters-counter-' . $settings['counter'],
		) );

		if ( 'text' !== $settings['button_style'] && isset( $settings['icon_position'] ) ) {
			$this->add_render_attribute( 'button-content', 'class', 'cmsmasters-icon-position-' . $settings['icon_position'] );
		}

		if ( 'relative' === $settings['counter'] ) {
			$this->add_render_attribute( 'button-content', 'class', 'cmsmasters-counter-type-' . $settings['counter_type'] );

			if ( isset( $settings['counter_position'] ) ) {
				$this->add_render_attribute( 'button-content', 'class', 'cmsmasters-counter-position-' . $settings['counter_position'] );
			}
		}

		if ( 'popup' === $settings['cart_type'] && 'none' === $settings['show_cart_on'] ) {
			if ( '' !== $settings['cart_url']['url'] ) {
				$this->add_render_attribute( 'button-content', $settings['cart_url'] );
			} else {
				$this->add_render_attribute( 'button-content', 'href', wc_get_cart_url() );
			}
		} else {
			$this->add_render_attribute( 'button-content', 'href', wc_get_cart_url() );
		}
		...
		if ( isset( $settings['button_text'] ) || 'true' === $settings['subtotal'] ) {
			...
			if ( isset( $settings['button_text'] ) ) {
				echo $this->get_button_text();
			}

			if ( 'true' === $settings['subtotal'] ) {
				echo $this->get_subtotal();
			}
		...
		if ( 'none' !== $settings['counter'] || 'text' !== $settings['button_style'] ) {
			...
			if ( 'none' !== $settings['counter'] ) {
				echo $this->get_counter();
			}

			if ( 'text' !== $settings['button_style'] ) {
				echo $this->get_icon_cart();
			}
		...
		if (
			...
			$this->add_render_attribute( 'cart-content', 'class', array(
				...
				'cmsmasters-button-type-' . $settings['cart_buttons_type'],
			) );

			if ( isset( $settings['show_cart_on'] ) ) {
				$this->add_render_attribute( 'cart-content', 'class', 'cmsmasters-show-cart-' . $settings['show_cart_on'] );
			}

			if ( 'canvas' === $settings['cart_type'] ) {
				$this->add_render_attribute( 'cart-content', 'class', 'cmsmasters-canvas-position-' . $settings['canvas_position'] );
			}

			if ( isset( $settings['product_image_position'] ) ) {
				$this->add_render_attribute( 'cart-content', 'class', 'cmsmasters-woo-cart-image-' . $settings['product_image_position'] );
			}

			if ( isset( $settings['product_price_type'] ) ) {
				$this->add_render_attribute( 'cart-content', 'class', 'cmsmasters-woo-cart-price-' . $settings['product_price_type'] );
			}

			$this->add_render_attribute( 'cart-wrapper', 'class', 'elementor-widget-cmsmasters-woo-cart__cart-wrapper' );

			if ( isset( $settings['close_position'] ) ) {
				$this->add_render_attribute( 'cart-wrapper', 'class', 'cmsmasters-position-' . $settings['close_position'] );
			}
	...
	public function get_cart_title() {
		...
		if ( '' !== $settings['cart_title'] ) {
			echo '<div class="elementor-widget-cmsmasters-woo-cart__cart-title">' . $settings['cart_title'] . '</div>';
		}
	}
	...
	public function get_cart_message() {
		...
		if ( '' !== $settings['cart_message'] ) {
			echo '<div class="elementor-widget-cmsmasters-woo-cart__cart-description">' . $settings['cart_message'] . '</div>';
		}
	}
