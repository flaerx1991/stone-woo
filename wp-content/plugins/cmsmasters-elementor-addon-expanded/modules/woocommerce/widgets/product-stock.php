	...
	protected function register_controls() {
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors' => array(
					'{{WRAPPER}} .elementor-widget-container, {{WRAPPER}} .stock' => 'text-align: {{VALUE}}',
				),
			)
		);
			...
			if ( 'default' !== $settings['availability_choose'] ) {
				if ( 'low_amount' !== get_option( 'woocommerce_stock_format' ) ) {
					...
					if ( $stock_quantity && '' !== $settings['availability_text'] ) {
						add_filter( 'woocommerce_get_availability_text', array( $this, 'custom_availability_text' ) );
					} elseif ( ! $stock_quantity && '' !== $settings['in_stock_text'] ) {
						add_filter( 'woocommerce_get_availability_text', array( $this, 'custom_availability_text_stock' ) );
					}
				}
			}
		}
