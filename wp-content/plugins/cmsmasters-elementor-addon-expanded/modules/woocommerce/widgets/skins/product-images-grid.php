	...
	protected function render_grid_images( $settings, $product ) {
		...
		if ( empty( $attachment_ids ) ) {
			if ( $product->get_image_id() ) {
				$attachment_ids[0] = $product->get_image_id();
			} else {
				return;
			}
		}
		...
		if (
			...
			$this->render_image( $settings, get_post_thumbnail_id(), $thumbnail_image_src );
		...
		if ( 1 < count( $attachment_ids ) ) {
			...
			foreach ( $attachment_ids as $attachment_id ) {
				...
				if ( $gallery_image_src ) {
					...
					$this->render_image( $settings, $attachment_id, $gallery_image_src );
