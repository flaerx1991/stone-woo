...
class Product_Reviews_Custom extends Product_Reviews_Base {
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors' => array(
					...
					'{{WRAPPER}} .cmsmasters-product-reviews' => '--input-icon-padding: {{RIGHT}}{{UNIT}};',
					'{{WRAPPER}} .cmsmasters-product-reviews' => '--textarea-icon-padding: {{TOP}}{{UNIT}};',
				),
			)
		);
	...
	public function editor_log_out_preview() {
		...
		$comment_text = ( ( ! $data['review_text'] ) ? esc_attr__( 'Review', 'cmsmasters-elementor' ) : $data['comment_text'] );
		...
		'<p class="comment-form-comment">' .
			( ( ! $is_placeholder ) ? '<label for="comment">' . $data['icon']['review'] . $comment_text . '</label>' : '' ) .
