...
class Product_Rating extends Base_Widget {
	...
	protected function render() {
		...
		if ( 'yes' === $settings['show_single_empty_rating'] || 0 < $product->get_rating_count() ) {
			...
			$empty_icon_value = Utils::get_render_icon( $settings[ 'empty_rating_icon' ], $attributes = array(), $with_wrap = false );
			$filled_icon_value = Utils::get_render_icon( $settings[ 'filled_rating_icon' ], $attributes = array(), $with_wrap = false );

			if ( ! empty( $empty_icon_value ) || ! empty( $filled_icon_value ) ) {
				$widget_class = 'elementor-widget-cmsmasters-woo-product-rating';
				...
				echo "<div class=\"{$widget_class}__content\">" .
					"<div class=\"{$widget_class}__html-empty\">" .
						$this->generate_rating_stars( $empty_icon ) .
						"<div class=\"{$widget_class}__html-filled\" style=\"width:{$filled_percent}%;\">" .
