...
if ( have_comments() ) {
	$reviews_nav = cmsmasters_get_standard_nav( $data, $parent_class );

	$review_title_tag = $data['settings']['custom_review_list_title_tag'];

	echo '<div class="' . esc_attr( $parent_class ) . '__wrapper">' .
		'<' . $review_title_tag . ' class="' . esc_attr( $parent_class ) . '__title">' . cmsmasters_get_review_title( $data ) . '</' . $review_title_tag . '>' .
		wp_kses_post( $reviews_nav ) .
...
comment_form( array(
	...
	'comment_field' => $comment_field .
		'<p class="comment-form-comment">' .
			( ( ! $is_placeholder ) ? "<label for=\"comment\">{$review_text}</label>" : '' ) .
