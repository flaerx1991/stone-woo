...
class Social_Counter extends Base_Widget {
		...
		foreach ( self::get_social_classes() as $social_name => $social_item_class ) {
			...
			$this->add_control(
				...
				array(
					...
					'condition' => array(
						'item_name!' => Items\Instagram::get_name(),
					),
				)
			);
	...
	protected function register_controls_notice() {
		...
		$this->add_control(
			...
			array(
				'type' => Controls_Manager::RAW_HTML,
				'raw' => __( 'followers will be shown if you have a business instagram account', 'cmsmasters-elementor' ),
				'content_classes' => 'elementor-panel-alert elementor-panel-alert-warning',
				'condition' => array(
					'type_instagram' => 'followers',
				),
			)
		);
	...
	public static function get_social_classes() {
		return array(
			...
			Items\Instagram::get_name() => Items\Instagram::get_class_full_name(),
	...
	public static function get_social_labels() {
		return array(
			...
			Items\Instagram::get_name() => Items\Instagram::get_label(),
	...
	public function get_social_item_class( $item_name ) {
		return Utils::get_if_isset( self::get_social_classes(), $item_name );
	}
	...
	public function get_numbers_by_format( $numbers = null, $format = '' ) {
		...
		switch ( $format ) {
			...
			case 'short':
			default:
				return Utils::short_number( $numbers );
		}
	}
