...
abstract class Base extends ElementorSkinBase {
	...
	protected function get_social_item_attrs() {
		...
		$social_item_attrs = array(
			...
			'data-name' => $social_item::get_name(),
		);
		...
		if ( ! $cache_checked_expire ) {
			$social_item_attrs['data-cache-id'] = $social_item->get_cache_id();
	...
	protected function render_social() {
		...
		$social_link_attr = array(
			...
			'href' => $social_item->get_profile_url(),
			'class' => array(
				'social-link',
			),
			'title' => $social_item::get_label(),
		);

		echo '<div ' . ElementorUtils::render_html_attributes( $this->get_social_item_attrs() ) . '>' .
			'<a ' . ElementorUtils::render_html_attributes( $social_link_attr ) . '>' .
	...
	protected function render_title() {
		...
		<div class="social-title">
			<span>
				<?php echo $this->parent->get_social_item()->get_title(); ?>
			</span>
		</div>
		<?php
	}
		...
		$this->parent->add_render_attribute(
			...
			array(
				'class' => array(
					...
					"cmsmasters-social-counter--{$this->get_id()}",
				),
			)
		);
