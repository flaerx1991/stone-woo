...
class Gallery extends Base_Widget {
	...
	protected function render() {
		...
		if ( ! empty( $galleries ) ) {
			...
			echo '<div ' . $this->get_render_attribute_string( 'gallery_container' ) . '>';
				...
				foreach ( $gallery_settings as $index => $item ) {
					...
					if ( ! is_null( $attachment ) ) {
						...
						if ( ! is_null( $attachment ) ) {
							...
							$this->add_render_attribute( 'gallery_item_image_' . $unique_index, array( 'alt' => $image_data['alt'] ) );
						}
						...
						echo '<' . $gallery_item_tag . ' ' . $this->get_render_attribute_string( 'gallery_item_' . $unique_index ) . '>
							<div ' . $this->get_render_attribute_string( 'gallery_item_image_' . $unique_index ) . '>';

						if ( 'yes' === $settings['overlay_background'] ) {
							echo '<div ' . $this->get_render_attribute_string( 'gallery_item_background_overlay' ) . ' ></div>';
						}

						echo '</div>';

						if ( 'none' !== $settings['overlay_content'] ) {
							echo '<div ' . $this->get_render_attribute_string( 'gallery_item_content' ) . '>';

							if ( $has_title || $has_description ) {
								$image_data = $this->get_image_data( $attachment );

								echo "<div class=\"{$base_class}__text-box\">";
								...
								if ( $has_title ) {
									...
									if ( ! empty( $title ) ) {
										echo '<div ' . $this->get_render_attribute_string( 'gallery_item_title' ) . '>' . $title . '</div>';
									}
								}
								...
								if ( $has_description ) {
									...
									if ( ! empty( $description ) ) {
										echo '<div ' . $this->get_render_attribute_string( 'gallery_item_description' ) . '>' . $description . '</div>';
									}
								}
							...
							if ( 'icon' === $settings['overlay_content'] ) {
								echo '<span ' . $this->get_render_attribute_string( 'gallery_item_icon' ) . '>';
	...
	protected function get_multiple_galleries() {
		...
		if ( ! $settings['hide_filter_bar'] ) {
			...
			echo '<div ' . $this->get_render_attribute_string( 'titles-outer' ) . '>
				<div ' . $this->get_render_attribute_string( 'titles-container' ) . '>';

			if ( $settings['show_all_galleries'] ) {
				echo "<a data-gallery-index=\"all\" class=\"{$base_class}__bar-item {$base_class}__title\">{$settings['show_all_galleries_label']}</a>";
			}
			...
			foreach ( $settings['galleries'] as $index => $gallery ) {
				...
				echo "<a data-gallery-index=\"{$index}\" class=\"{$base_class}__bar-item {$base_class}__title\">{$gallery['gallery_title']}</a>";
