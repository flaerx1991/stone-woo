...
use CmsmastersElementor\Plugin as Addon;
...
class Post_Media extends Base_Widget {
	...
	/**
	 *
	 * Initializing the Addon `post media` widget class.
	 *
	 * @since 1.0.0
	 *
				...
				array(
					...
					'conditions' => array(
						...
						'terms' => array(
							...
							// array(
							// 	'name' => '__dynamic__[empty_image_placeholder]',
							// 	'operator' => '!==',
							// 	'value' => '',
							// ), // TODO https://i.imgur.com/GTssrFM.png
						),
					),
				)
			);
	...
	public function get_featured_image_controls( $type = 'standard' ) {
		$dynamic_tags = Addon::elementor()->dynamic_tags;
	...
	protected function render() {
		...
		if ( false === $format || 'image' === $format ) {
			...
			if ( ! empty( $gallery_data ) ) {
				...
				$this->slider->render( function() use ( $gallery_id, $settings, $format ) {
					foreach ( $gallery_id as $key => $id ) {
						...
						if ( $link ) {
							$this->add_render_attribute( 'link_' . $id, array(
								'href' => $link,
								'data-elementor-open-lightbox' => $settings[ $format . '_open_lightbox' ],
						...
						echo '<figure class="swiper-slide-inner">';
							echo $link ? '<a ' . $this->get_render_attribute_string( 'link_' . $id ) . '>' : '';
								echo '<img class="swiper-slide-image" src="' . esc_attr( $image_url ) . '" alt="' . $alt_text . '" />';
							echo $link ? '</a>' : '';
						echo '</figure>';
			...
			$audio_hosted_url = wp_get_attachment_url( $audio_hosted_id );
			$audio_embed_url = $this->get_post_meta( 'audio_link_embedded' );

			if ( 'hosted' === $post_audio_type && false !== $audio_hosted_url ) {
				echo wp_audio_shortcode( $this->hosted_audio_attr( $audio_hosted_url ) );
	...
	private function cmsmasters_render_video( $settings, $video_url, $post_video_type, $provider ) {
		...
		$lightbox = ' elementor-open-' . ( $lightbox_check ? 'lightbox' : 'inline' );

		$this->add_render_attribute( 'video-wrapper', 'class', 'elementor-widget-cmsmasters-post-media__container' . $lightbox );

		$this->add_render_attribute( 'video-inner', 'class', 'elementor-widget-cmsmasters-post-media__wrap elementor-fit-aspect-ratio' );
		...
		echo '<div ' . $this->get_render_attribute_string( 'video-wrapper' ) . '>' .
			'<div ' . $this->get_render_attribute_string( 'video-inner' ) . '>' .
		...
		if ( $this->has_image_placeholder() ) {
			$this->add_render_attribute( 'image-placeholder', 'class', 'elementor-widget-cmsmasters-post-media__image-placeholder' );
			...
			if ( 'yes' === $settings['show_play_icon'] && ! empty( $settings['play_icon']['value'] ) ) {
				echo '<div class="elementor-widget-cmsmasters-post-media__play-icon' . ( ( 'yes' === $settings['play_icon_effect'] ) ? ' disable_effect' : '' ) . '" role="button">';
	...
	private function render_hosted_video() {
		...
		?>
		<video class="elementor-widget-cmsmasters-post-media__hosted" src="<?php echo esc_url( $video_url ); ?>" <?php echo Utils::render_html_attributes( $video_params ); ?>></video>
		<?php
	}
