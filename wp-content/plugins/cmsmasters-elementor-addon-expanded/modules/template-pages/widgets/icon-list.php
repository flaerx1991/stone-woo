...
class Icon_List extends Base_Widget {
	...
	protected function register_list_dynamic_content() {
		$this->add_control(
			...
			array(
				...
				'dynamic' => array(
					'active' => true,
					'default' => Plugin::elementor()->dynamic_tags->tag_data_to_tag_text( null, 'cmsmasters-acf-repeater-text' ),
				),
		...
		$this->add_control(
			...
			array(
				...
				'dynamic' => array(
					'active' => true,
					'default' => Plugin::elementor()->dynamic_tags->tag_data_to_tag_text( null, 'cmsmasters-acf-repeater-url' ),
				),
				'condition' => array( 'data_type' => 'dynamic' ),
			)
		);
	...
	protected function render() {
		...
		if ( ! empty( $settings['title'] ) ) {
			...
			echo "<{$tag} " . $this->get_render_attribute_string( 'list_title' ) . ">{$settings['title']}</{$tag}>";
		}
		...
		<ul <?php echo $this->get_render_attribute_string( 'icon_list' ); ?>>
			...
			foreach ( $list_data as $index => $item ) {
				...
				$this->add_render_attribute( $repeater_text_setting_key, 'class', "{$item_class}-text-inner" );
					...
					<span <?php echo $this->get_render_attribute_string( $repeater_text_setting_key ); ?>>
						<span <?php echo $this->get_render_attribute_string( 'text' ); ?>>
							...
							if ( $has_item_link && 'text' === $link_type ) {
								echo '<a ' . $this->get_render_attribute_string( $link_key ) . '>' . $item['text'] . '</a>';
							} else {
								echo $item['text'];
							}
							?>
