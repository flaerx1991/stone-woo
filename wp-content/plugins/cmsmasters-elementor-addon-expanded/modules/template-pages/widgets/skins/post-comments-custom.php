...
class Post_Comments_Custom extends Post_Comments_Base {
		...
		$this->add_control(
			...
			array(
				...
				'condition' => array(
					...
					'custom_comment_border_border!' => 'default',
				),
			)
		);
	...
	protected function style_reply() {
		...
		foreach ( $link_states  as $tabs_key => $label ) {
			...
			$this->parent->start_injection( array( 'of' => "custom_reply_bg_group_{$tabs_key}_background" ) );
		...
		foreach ( $link_edit_states  as $tabs_key => $label ) {
			...
			$this->parent->start_injection( array( 'of' => "custom_edit_bg_group_{$tabs_key}_background" ) );
	...
	protected function style_form_elements() {
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors' => array(
					...
					'{{WRAPPER}} .cmsmasters-single-post-comments' => '--input-icon-padding: {{RIGHT}}{{UNIT}};',
					'{{WRAPPER}} .cmsmasters-single-post-comments' => '--textarea-icon-padding: {{TOP}}{{UNIT}};',
				),
			)
		);
		...
		foreach ( $link_states as $main_key => $label ) {
			...
			$this->add_group_control(
				CmsmastersControls::BUTTON_BACKGROUND_GROUP,
				array(
					'name' => "submit_button_background_group_{$main_key}",
					'exclude' => array( 'color' ),
					'selector' => $gradient_selector,
				)
			);

			$this->parent->start_injection( array( 'of' => "custom_submit_button_background_group_{$main_key}_background" ) );
	...
	public function editor_log_out_preview() {
		...
		'<div class="cmsmasters-input-wrap"><p class="comment-form-author">' .
			( ( ! $is_placeholder ) ? '<label for="author">' . $data['icon']['name'] . $name_text . ' *</label> ' : '' ) .
			'<input type="text" id="author" name="author" size="35" placeholder="' .
				( ( $is_placeholder ) ? $name_text . ' *' : '' ) .
			'" />' .
			( ( $is_placeholder ) ? $data['icon']['name'] : '' ) .
		'</p>' .
		'<p class="comment-form-email">' .
			( ( ! $is_placeholder ) ? '<label for="email">' . $data['icon']['email'] . $email_text . ' *</label> ' : '' ) .
			'<input type="text" id="email" name="email" size="35" placeholder="' .
