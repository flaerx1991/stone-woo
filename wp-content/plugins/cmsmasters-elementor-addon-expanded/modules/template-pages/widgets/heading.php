...
class Heading extends Base_Widget {
	...
	protected function register_controls() {
		...
		$this->add_responsive_control(
			...
			array(
				...
				'default' => '',
				'selectors' => array(
					'{{WRAPPER}} .cmsmasters-widget-title__heading, {{WRAPPER}} .elementor-widget-container' => 'text-align: {{VALUE}};',
				),
			)
		);
	...
	protected function render() {
		...
		if ( ! empty( $settings['title_hover_animation'] ) ) {
			$this->add_render_attribute( 'title', 'class', 'elementor-animation-' . $settings['title_hover_animation'] );
		}
