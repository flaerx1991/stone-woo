...
class Author_Box extends Base_Widget {
	...
	protected function register_controls() {
		...
		foreach ( $button_states  as $button_key => $label ) {
			...
			$this->add_group_control(
				...
				array(
					'name' => "button_background_group{$button_id_key}",
					'exclude' => array( 'color' ),
					'selector' => $gradient_selector,
				)
			);

			$this->start_injection( array( 'of' => "button_background_group{$button_id_key}_background" ) );
	...
	private function print_social_list_add() {
		...
		foreach ( $icon_list_add as $item ) {
			$this->add_render_attribute(
				"social-{$item}-link",
				array( 'href' => get_the_author_meta( "user_{$item}", get_post( get_the_ID() )->post_author ) )
			);
			...
			if (
				(
					...
					! empty( $settings[ 'social_media_icon_' . $item ]['value'] )
				) ||
				! empty( $get_social_title )
			) {
				echo "<li id=\"{$social_item_class}-{$item}\" class=\"{$social_item_class}\">
					<a {$this->get_render_attribute_string( 'social-' . $item . '-link' )}>";

						if ( 'icon' === $settings['social_media_type'] ) {
							Icons_Manager::render_icon( $settings[ 'social_media_icon_' . $item ] );
						} else {
							echo $get_social_title;
						}
	...
	private function print_social_list( $type ) {
		...
		foreach ( $show_social_fields as $key => $field ) {
			...
			if ( '' !== $social_url ) {
				...
				if ( isset( $settings[ 'social_media_icon_' . $key ] ) && $settings[ 'social_media_icon_' . $key ]['value'] || $social_title ) {
					echo '<li ' . $this->get_render_attribute_string( $social_setting_attr ) . '>' .
						"<a href=\"{$social_url}\">";

					if ( isset( $settings[ 'social_media_icon_' . $key ] ) && '' === $social_title ) {
						Icons_Manager::render_icon( $settings[ 'social_media_icon_' . $key ] );
					} else {
						echo $social_title;
					}
	...
	private function print_social_custom_list( $type ) {
		...
		foreach ( $social as $index => $item ) :
			...
			if ( '' !== $item[ 'custom_social_media_' . $type . '_link' ]['url'] ) {
				...
				echo "<li {$this->get_render_attribute_string( $social_setting_attr )}>
					<a {$social_url}{$get_social_icon_class}>";

				if ( isset( $item[ 'custom_social_media_' . $type ]['library'] ) && 'svg' !== $item[ 'custom_social_media_' . $type ]['library'] ) {
					echo $get_social_title;
				} else {
					Icons_Manager::render_icon( $item[ 'custom_social_media_' . $type ] );
				}
			...
			switch ( $author_box ) {
				case 'name':
					if ( ( ! $is_custom_src ) || ( $is_custom_src && ! empty( $author['display_name'] ) ) ) {
						...
						if ( ! empty( $link_to ) || $is_custom_src ) {
							$author_link_attrs = $this->get_render_attribute_string( 'author-link' );

							$author_name = "<{$link_tag} {$author_link_attrs}>{$author_name}</{$link_tag}>";
						}

						if ( isset( $inline_wrap ) ) {
							if ( 'social' !== $inline_wrap ) {
								echo '<div class="cmsmasters-title-inline-wrapper">';

								if ( 'yes' === $inline_title_social && 'yes' === $avatar_inline ) {
									$this->get_avatar( $author, $link_tag );
								}
							}
						}

						echo "<{$settings['author_name_tag']} class=\"elementor-widget-cmsmasters-author-box__name\">{$author_name}</{$settings['author_name_tag']}>";
				...
				case 'biography':
					if ( ( ! $is_custom_src ) || ( $is_custom_src && ! empty( $author['bio'] ) ) ) {
						echo "<div class=\"elementor-widget-cmsmasters-author-box__bio\">{$author['bio']}</div>";
					}
				...
				case 'link':
					if (
						...
						$button_attrs = $this->get_render_attribute_string( 'button' );

						echo "<div class=\"elementor-widget-cmsmasters-author-box__button-wrap\">
							<a {$button_attrs}>{$settings['link_text']}</a>
						</div>";
					}
	...
	public function get_avatar( $author, $link_tag ) {
		...
		if (
			...
			$author_link_attrs = $this->get_render_attribute_string( 'author-link' );
			$avatar_attrs = $this->get_render_attribute_string( 'avatar' );

			echo "<{$link_tag} {$author_link_attrs} class=\"elementor-widget-cmsmasters-author-box__avatar\">
				<img {$avatar_attrs}>
			</{$link_tag}>";
		}
	}
