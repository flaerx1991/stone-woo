...
class Video_Playlist extends Base_Widget {
	...
	/**
	 * Retrieve the list of style the video playlist widget depended on.
	 *
	...
	protected function register_controls() {
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors' => array(
					'{{WRAPPER}} .elementor-widget-cmsmasters-video-playlist__canvas_inner' => 'padding-bottom: {{SIZE}}{{UNIT}};',
					'{{WRAPPER}} .elementor-widget-cmsmasters-video-playlist__item-thumb-ratio' => 'padding-bottom: {{SIZE}}{{UNIT}};',
					'{{WRAPPER}}.cmsmasters-list-orientation-vertical .elementor-widget-cmsmasters-video-playlist__container' => 'height: {{SIZE}}{{UNIT}};',
				),
		...
		$this->add_control(
			...
			array(
				...
				'default' => '',
				'frontend_available' => true,
				'prefix_class' => 'cmsmasters-playlist-item-aspect-ratio-',
			)
		);
		...
		$this->add_responsive_control(
			...
			array(
				...
				'selectors' => array(
					'{{WRAPPER}} .elementor-widget-cmsmasters-video-playlist__list .elementor-widget-cmsmasters-video-playlist__item-thumb-ratio' => 'padding-bottom: {{SIZE}}{{UNIT}};',
				),
	...
	protected function render() {
		...
		$widget_name = 'elementor-widget-cmsmasters-video-playlist';
		$list = $settings['playlist'];

		echo '<div class="' . $widget_name . '__container">' .
			'<div class="' . $widget_name . '__canvas">' .
				'<div class="' . $widget_name . '__canvas_inner elementor-fit-aspect-ratio">' .
					'<div class="' . $widget_name . '__canvas-cover">';
		...
		if ( 'yes' === $settings['show_play_icon'] && ! empty( $settings['play_icon']['value'] ) ) {
			echo '<div class="' . $widget_name . '__play-icon' . ( 'yes' === $settings['play_icon_effect'] ? ' disable_effect' : '' ) . '" role="button">';
				Icons_Manager::render_icon( $settings['play_icon'] );

				echo '<span class="elementor-screen-only">' .
					__( 'Play Video', 'cmsmasters-elementor' ) .
				'</span>' .
			'</div>';
		}

					echo '</div>' .
					'<div class="' . $widget_name . '__canvas-overlay"></div>' .
				'</div>' .
			'</div>' .
			'<div class="' . $widget_name . '__list">';

		if ( 'yes' === $settings['show_list_heading'] ) {
			$this->print_item_heading( $widget_name, $settings, $list );
		}

				echo '<div class="' . $widget_name . '__list-items">' .
					'<div class="' . $widget_name . '__list-items-content">';

						$this->print_item_item( $widget_name, $list, $settings );
	 ...
	 */
	public function print_item_item( $widget_name, $list, $settings ) {
		foreach ( $list as $index => $item ) {
			if ( '' !== $item['url'] ) {
				...
				$author_url = ! empty( $video_data['author_url'] ) ? $video_data['author_url'] : '';
				...
				printf(
					'<div class="' . $widget_name . '__item%2$s" %1$s>',
				...
				if ( 'yes' === $settings['show_numbering'] && 'vertical' === $settings['list_orientation'] ) {
					$this->print_item_index( $widget_name, $index, $hide, $settings );
				}

				if ( '' !== $thumb ) {
					$this->print_item_thumb( $widget_name, $hide, $thumb, $title, $duration, $index );
				}
				...
				if (
					...
					$this->print_item_content( $item, $widget_name, $title, $author_name, $author_tag, $subtitle, $subtitle_link, $hide );
				}
	 ...
	 * @since 1.0.0
	 */
	public function print_item_heading( $widget_name, $settings, $list ) {
		echo '<div class="' . $widget_name . '__heading">' .
			'<div class="' . $widget_name . '__heading-content">' .
				'<div class="' . $widget_name . '__heading-title">';
		...
		if ( '' !== $settings['list_heading_text'] ) {
			...
			echo esc_html__( 'Video PlayList', 'cmsmasters-elementor' );
				...
				echo esc_html( $this->__video_counter( $settings, $list ) );
	 ...
	 * @since 1.0.0
	 */
	public function print_item_index( $widget_name, $index, $hide, $settings ) {
		$list_orientation = $settings['list_orientation'];
		$hide_class = '';
		$sep = ( '' !== $hide['numbering'] || '' !== $hide['image'] ? ' ' : '' );

		if ( 'horizontal' === $list_orientation && $hide['image'] !== $hide['numbering'] ) {
			$hide_class = $hide['numbering'] . $sep . $hide['image'];
		} else {
			$hide_class = $hide['numbering'];
		}

		echo '<div class="' . $widget_name . '__item-numbering' . esc_attr( $hide_class ) . '">' .
			'<div class="' . $widget_name . '__item-numbering-num">' .
				esc_html( $index + 1 ) .
			'</div>' .
		'</div>';
	}
	 ...
	 * @since 1.0.0
	 */
	public function print_item_content( $item, $widget_name, $title, $author_name, $author_tag, $subtitle, $subtitle_link, $hide ) {
		echo '<div class="' . $widget_name . '__item-content">';

		if ( '' !== $title ) {
			echo '<div class="' . $widget_name . '__item-content-title">' .
				esc_html( $title ) .
			'</div>';
		}

		if ( '' !== $author_name || '' !== $subtitle ) {
			echo '<div class="' . $widget_name . '__item-content-author' . esc_attr( $hide['subtitle'] ) . '">';
			...
			$subtitle_tag = ( '' !== $subtitle_link ? 'a href="' . $subtitle_link . '"' . $subtitle_attr : 'span' );
	 ...
	 * @since 1.0.0
	 */
	public function print_item_thumb( $widget_name, $hide, $thumb, $title, $duration, $index ) {
		$settings = $this->get_settings_for_display();

		echo '<div class="' . esc_attr( $widget_name ) . '__item-thumb' . esc_attr( $hide['image'] ) . '">' .
			'<div class="' . esc_attr( $widget_name ) . '__item-status">';
			...
			echo'</div>' .
			'<div class="' . esc_attr( $widget_name ) . '__item-thumb-ratio elementor-fit-aspect-ratio cmsmasters-fit-aspect-ratio">';
		...
		if ( 'yes' === $settings['show_numbering'] && 'horizontal' === $settings['list_orientation'] ) {
			$this->print_item_index( $widget_name, $index, $hide, $settings );
				...
				echo '<img src="' . esc_attr( $thumb ) . '" 
					...
					class="' . esc_attr( $widget_name ) . '__item-thumb-img">';

		if ( 'yes' === $settings['show_duration'] && $duration ) {
			echo '<div class="' . $widget_name . '__item-duration' . esc_attr( $hide['duration'] ) . '">' .
				esc_html( $duration ) .
			'</div>';
		}
	...
	/**
	 * Get video data attributes.
	 *
	 * @var sting
	 *
	 * @since 1.0.0
	 * @since 1.1.0 Fixed assignment `$size` and `$unit` variable. Added check on empty.
	 * @since 1.5.0 Fixed quotes in data-html.
	 */
	public function __get_video_data_atts( $video_data, $item, $settings, $index ) {
		...
		$data = array(
			...
			'data-html' => str_replace( array( '"', "'" ), '', wp_json_encode( $this->adjust_height( $video_data['html'] ) ) ),
			'data-height' => $size . $unit,
	...
	public function __video_counter( $settings, $list ) {
		...
		if ( '' !== $settings['list_heading_counter_suffix'] ) {
			$suffix_value = $settings['list_heading_counter_suffix'];
		} else {
			if ( 1 < count( $list ) ) {
				$suffix_value = esc_html__( 'videos', 'cmsmasters-elementor' );
			} else {
				$suffix_value = esc_html__( 'video', 'cmsmasters-elementor' );
			}
		}

		$suffix = sprintf(
			' <span class="elementor-widget-cmsmasters-video-playlist__counter-suffix">%1$s</span>',
			$suffix_value
		);

		printf(
			'<div class="elementor-widget-cmsmasters-video-playlist__counter">' .
