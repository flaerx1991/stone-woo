...
/**
 * Addon video stream widget.
 *
 * Addon widget that displays video stream.
 *
 * @since 1.0.0
 */
class Video_Stream extends Base_Widget {
	...
	protected function register_controls() {
		...
		$breakpoints = AddonUtils::get_breakpoints();
	...
	protected function render() {
		...
		$open_lightbox = ' elementor-open-' . ( $lightbox ? 'lightbox' : 'inline' );
		...
		$show_chat_cl = ( 'both' === $show_video_chat ? ' show_video_chat' : '' );
		...
		echo '<div class="elementor-widget-cmsmasters-video-stream__container' . $open_lightbox . $minimize_always . $show_chat_cl . '">' .
		...
		if ( ! $lightbox ) {
			...
			echo '<iframe 
				...
				$lazy_load . '="' . $this->get_video() . '" 
				allowfullscreen 
				frameborder="0" 
				scrolling="no" 
				title="' . $settings['video_type'] . ' video player"></iframe>';
	...
	public function get_video_chat_params() {
		...
		echo '<div class="elementor-widget-cmsmasters-video-stream__video-chat">' .
			'<iframe class="elementor-widget-cmsmasters-video-stream__live-chat" ' . $src . ' title="' . $video_type . ' live chat"></iframe>' .
		'</div>';
	}
	...
	public function get_youtube_chat_params( $video_id ) {
		...
		$url = $host . $video_id . '&embed_domain=' . str_replace( 'www.', '', $_SERVER['SERVER_NAME'] ) . '"';
	...
	protected function cover_image() {
		...
		if ( ! empty( $settings['cover_image']['id'] ) ) {
			...
			if ( 'yes' === $settings['show_play_icon'] && ! empty( $settings['play_icon']['value'] ) ) {
				$disable_effect = '' === $settings['play_icon_effect'] ? ' disable_effect' : '';

				echo '<div class="elementor-widget-cmsmasters-video-stream__play-icon' . $disable_effect . '" role="button">';
