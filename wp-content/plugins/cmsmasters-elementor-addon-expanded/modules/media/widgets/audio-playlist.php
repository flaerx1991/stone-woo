...
class Audio_Playlist extends Base_Widget {
	...
	protected function print_audio_poster() {
		...
		$widget_name = 'elementor-widget-cmsmasters-audio-playlist';

		echo '<div class="' . $widget_name . '__poster">' .
	...
	protected function print_control_button( $button, $title, $add_attr, $add_icon, $icon_class, $icon ) {
		...
		echo '<div 
			class="' . $widget_name . '__controls-button cmsmasters-player-' . $button . ( 'volume' === $button ? ' cmsmasters-volume-up-active' : '' ) . '" 
		...
		if ( $add_icon ) {
			Icons_Manager::render_icon( array(
				...
				'library' => 'fa-solid',
			), array( 'class' => $widget_name . $icon_class ) );
			...
			if ( 'volume' === $button ) {
				Icons_Manager::render_icon( array(
					...
					'library' => 'fa-solid',
				), array( 'class' => $widget_name . '__volume-icon cmsmasters-volume-down' ) );

				Icons_Manager::render_icon( array(
					'value' => 'fas fa-volume-off',
					'library' => 'fa-solid',
				), array( 'class' => $widget_name . '__volume-icon cmsmasters-volume-off' ) );

				Icons_Manager::render_icon( array(
					'value' => 'fas fa-volume-mute',
					'library' => 'fa-solid',
				), array( 'class' => $widget_name . '__volume-icon cmsmasters-volume-mute' ) );
			}
		}
	...
	protected function print_current_time() {
		...
		echo '<div class="' . $widget_name . '__current-time" title="' . esc_attr__( 'Current Time', 'cmsmasters-elementor' ) . '">' .
			'<span class="' . $widget_name . '__current-time-value">' .
				esc_html( '00:00' ) .
			'</span>' .
		'</div>';
	}
	...
	protected function print_total_time() {
		...
		echo '<div class="' . $widget_name . '__total-time" title="' . esc_attr__( 'Total Time', 'cmsmasters-elementor' ) . '">' .
	...
	protected function print_volume() {
		...
		echo '<div class="' . $widget_name . '__volume-wrap">' .
			'<div class="' . $widget_name . '__volume-inner">';
				...
				$this->print_control_button( 'volume', '100%', '', true, '__volume-icon cmsmasters-volume-up', 'volume_up_icon' );
	...
	protected function print_control_buttons_left() {
		...
		if ( 'both' === $settings['audio_control_button_prev_next'] && '1' < count( $audio_list ) ) {
			$this->print_control_button( 'prev', 'Prev', 'disabled', true, '__prev-icon', 'prev_icon' );
		}
		...
		if ( 'none' !== $settings['audio_control_button_prev_next'] && '1' < count( $audio_list ) ) {
			$this->print_control_button( 'next', 'Next', '', true, '__next-icon', 'next_icon' );
		}
	...
	protected function print_advanced() {
		...
		$speed_value = 'cmsmasters-player-speed-';
		$list = $settings['audio_list'];
		...
		if ( 'yes' === $settings['audio_advanced_download'] || 'yes' === $settings['audio_advanced_speed'] ) {
			echo '<div class="' . $widget_name . '__advanced">';
				...
				Icons_Manager::render_icon( array(
					...
					'class' => $widget_name . '__advanced-icon',
					'title' => esc_attr__( 'Advanced', 'cmsmasters-elementor' ),
				) );

				echo '<div class="' . $widget_name . '__advanced_inner">';
			...
			if ( 'yes' === $settings['audio_advanced_speed'] ) {
				...
				echo '</span>' .
					'<span class="' . $widget_name . '__speed-title-wrap">' .
						'<span class="' . $widget_name . '__speed-title">' . esc_html__( 'Speed:', 'cmsmasters-elementor' ) . '</span>' .
						'<span class="' . $widget_name . '__speed-rate">' . esc_html__( 'Normal', 'cmsmasters-elementor' ) . '</span>' .
					'</span>' .
				'</span>';
			}
	...
	protected function print_audio_item( $item, $active ) {
		...
		$widget_name = 'elementor-widget-cmsmasters-audio-playlist';
		$playlist_item_separator = ( 'yes' === $settings['audio_playlist_item_separator'] ? ' cmsmasters-playlist-item-separator' : '' );

		echo '<li class="' . $widget_name . '__playlist_item' . $playlist_item_separator . ( $active ? ' cmsmasters-active-track' : '' ) . '">' .
	...
	protected function print_audio_playlist() {
		...
		echo '<div class="' . $widget_name . '__playlist ' . $settings['audio_playlist_type'] . '">' .
	...
	protected function print_audio_medium_player() {
		...
		echo '</div>' .
		'<div class="' . $widget_name . '__player_right' . ( empty( $poster ) ? ' cmsmasters-empty-poster' : '' ) . '">';
