...
class Audio extends Base_Widget {
	...
	protected function render() {
		...
		add_filter( 'oembed_result', array( $this, 'filter_' . $this->_current_instance['audio_type'] . '_result' ), 50, 3 );
		$video_html = wp_oembed_get( $audio_type_link['url'], wp_embed_defaults() );
		remove_filter( 'oembed_result', array( $this, 'filter_' . $this->_current_instance['audio_type'] . '_result' ), 50 );

		if ( $video_html || 'deezer' === $audio_type ) {
	...
	protected function print_current_time() {
		$widget_name = 'elementor-widget-cmsmasters-audio';
	...
	protected function print_volume() {
		...
		echo '<div class="' . $widget_name . '__volume-wrap">' .
			'<div class="' . $widget_name . '__volume-inner">' .
				'<div class="' . $widget_name . '__controls-button cmsmasters-player-volume cmsmasters-volume-up-active" title="' . esc_attr__( '100%', 'cmsmasters-elementor' ) . '">';

					Icons_Manager::render_icon( array(
						'value' => 'fas fa-volume-up',
						'library' => 'fa-solid',
					), array( 'class' => $widget_name . '__volume-icon cmsmasters-volume-up' ) );
				...
				echo '</div>' .
				'<div class="' . $widget_name . '__volume-progress-wrap">' .
	...
	protected function print_audio_hosted_player() {
		...
		echo '<div class="' . $widget_name . '__player_wrap">' .
			'<div class="' . $widget_name . '__player-bg">' .
				...
				'<audio class="' . $widget_name . '__player-audio" preload="auto" itemprop="audio" tabindex="0" type="audio/mpeg">' .
					'<source class="' . $widget_name . '__player-source" type="audio/mp3" autoplay="autoplay" src="' . $link . '">' .
				'</audio>' .
				'<div class="' . $widget_name . '__player">' .
					'<div class="' . $widget_name . '__controls-button cmsmasters-player-play" title="Play">';
		...
		if ( '' !== $settings['hosted_additional_info'] ) {
			echo '<div class="' . $widget_name . '__additional_info">' . esc_html( $settings['hosted_additional_info'] ) . '</div>';
		}
