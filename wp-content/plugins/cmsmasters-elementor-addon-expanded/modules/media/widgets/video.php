 ...
 * Addon video widget.
 *
 * Addon widget that displays video.
 *
 * @since 1.0.0
 */
class Video extends Base_Widget {
	...
	protected function register_controls() {
		...
		$this->add_group_control(
			...
			array(
				'name' => 'css_filters',
				'selector' => '{{WRAPPER}} .elementor-widget-cmsmasters-video__cover-image',
				'condition' => array( 'cover_image[url]!' => '' ),
			)
		);
	...
	protected function render() {
		...
		$lightbox = ' elementor-open-' . ( $settings['lightbox'] ? 'lightbox' : 'inline' );
		$minimize_always = ( ( '' === $settings['video_minimize_overlay'] ) ? ' minimize_always' : '' );

		echo '<div class="elementor-widget-cmsmasters-video__container' . $lightbox . $minimize_always . '">' .
			'<div class="elementor-widget-cmsmasters-video__wrap elementor-fit-aspect-ratio">' .
				'<div class="' . ( ! $settings['lightbox'] ? 'elementor-widget-cmsmasters-video__inner' : '' ) . '">' .
					'<span class="elementor-widget-cmsmasters-video__close-button eicon-close"></span>';
	...
	protected function print_cover_image( $video_type, $video_url ) {
		...
		$this->add_render_attribute( 'cover-image', 'class', 'elementor-widget-cmsmasters-video__cover-image' );
		...
		if ( 'yes' === $settings['show_play_icon'] && ! empty( $settings['play_icon']['value'] ) ) {
			echo '<div class="elementor-widget-cmsmasters-video__play-icon' . ( 'yes' === $settings['play_icon_effect'] ? ' disable_effect' : '' ) . '" role="button">';
	...
	public function get_facebook_params() {
		...
		echo '<iframe class="elementor-video-iframe"' . $src . $host . $url . implode( '&', $params ) . '" frameborder="0" scrolling="no" title="' . __( 'Facebook Video Player', 'cmsmasters-elementor' ) . '"></iframe>';
	}
	...
	public function get_twitch_params() {
		...
		echo '<iframe class="elementor-video-iframe" ' . $src . $url . '" frameborder="0" layout="video" scrolling="no" title="' . __( 'Twitch Video Player', 'cmsmasters-elementor' ) . '"></iframe>';
	}
