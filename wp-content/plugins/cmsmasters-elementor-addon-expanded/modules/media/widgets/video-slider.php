...
class Video_Slider extends Base_Widget {
	...
	public function get_script_depends() {
		return array_merge( array( 'jquery-ui-draggable' ), parent::get_script_depends() );
	}
	...
	protected function print_video_slider() {
		...
		echo '<div class="elementor-widget-cmsmasters-video-slider__container' . $arrows_nav_icon_devices . $arrows_nav_text_devices . '">' .
			'<div class="elementor-widget-cmsmasters-video-slider__wrap cmsmasters_swiper_content">';

				$this->print_video_slide( $settings, $list );

		if ( count( $list ) > 1 && 'video' === $settings['arrows_nav_position'] ) {
			echo $this->print_arrows();
		}

			echo '</div>';

			$this->print_video_info_box( $settings, $list );
	 ...
	 * @since 1.0.0
	 */
	protected function print_video_slide( $settings, $list ) {
		echo '<div class="swiper-wrapper">';

		foreach ( $list as $index => $slide ) {
			$this->slide_prints_count++;

			echo '<div class="elementor-repeater-item elementor-repeater-item-' . $slide['_id'] . ' swiper-slide" data-repeater-id="' . $slide['_id'] . '">';
				echo $this->get_video_slide( $slide, $settings, 'slide-' . $index . '-' . $this->slide_prints_count );
			echo '</div>';
		}
	 ...
	 */
	protected function get_video_slide( $slide, $settings, $element_key ) {
		$settings = $this->get_settings_for_display();
		...
		echo '<div ' . $this->get_render_attribute_string( $element_key . '-slide' ) . '>' .
			'<div class="elementor-widget-cmsmasters-video-slider__video-box">' .
				'<div class="elementor-widget-cmsmasters-video-slider__video-box-inner elementor-fit-aspect-ratio">';

					echo '<div class="elementor-widget-cmsmasters-video-slider__image-overlay">';

		if ( ! empty( $slide['cover']['id'] ) ) {
			echo $this->get_attachment_image_html( $slide, $settings['cover_size'], 'cover' );

			$play_button = $settings['play_button_icon'];

			if ( ! empty( $play_button['value'] ) ) {
				echo '<div class="elementor-widget-cmsmasters-video-slider__play-button' . ( 'yes' === $settings['play_button_blinking'] ? '' : ' cmsmasters-disable-effect' ) . '" role="button">';

					Icons_Manager::render_icon( $play_button, array( 'aria-hidden' => 'true' ) );

					echo '<span class="elementor-screen-only">' . __( 'Play Video', 'cmsmasters-elementor' ) . '</span>' .
				'</div>';
			}
		}
	...
	private function render_hosted_video( $slide ) {
		...
		$src = ( $slide['cover']['id'] ? 'data-lazy-load=' : 'src=' );

		echo '<video class="elementor-video cmsmasters-hosted-video" ' . $src . '"' . esc_url( $video_url ) . '" ' . Utils::render_html_attributes( array() ) . ' controls="controls"></video>';
	}
	...
	private function print_arrows() {
		...
		if ( $settings['arrows_nav_enable'] ) {
			...
			$arrows_nav_type = $settings['arrows_nav_type'];

			echo '<div class="cmsmasters_arrow_buttons">';

			foreach ( $arrow_button_align as $align ) {
				echo '<div class="cmsmasters_arrow_button_' . $align . ' cmsmasters_arrow_button">';

				if ( 'text' !== $arrows_nav_type ) {
					$arrow_buttons_icon = $settings[ 'arrows_nav_icon_' . $align ];

					echo '<span class="cmsmasters_arrow_button_icon">';

					if ( '' !== $arrow_buttons_icon['value'] ) {
						Icons_Manager::render_icon( $arrow_buttons_icon );
					} else {
	 ...
	 * @since 1.0.0
	 */
	protected function print_video_info_box( $settings, $list ) {
		$list = $settings['videos_list'];
		$info_box_responsive_devices = '';
		...
		echo '<div class="elementor-widget-cmsmasters-video-slider__content' . $info_box_responsive_devices . '">' .
	...
	private function get_video_info_box( $slide ) {
		...
		if ( $content_title || $content_subtitle || $content_author ) {
			...
			foreach ( $settings['info_variations'] as $item ) {
				switch ( $item ) {
					case 'author':
						echo $this->print_video_author( $slide );
						break;
					case 'subtitle':
						echo $this->print_video_subtitle( $slide );
						break;
					case 'title':
						echo $this->print_video_title( $slide );
						break;
				}
			}
						...
						<div class="elementor-widget-cmsmasters-video-slider__video-box-inner elementor-fit-aspect-ratio">
							<div class="elementor-widget-cmsmasters-video-slider__image-overlay"><#
								...
								if ( '' !== slide.cover.id ) {
									...
									if ( '' !== playButton.value ) {
										#><div class="elementor-widget-cmsmasters-video-slider__play-button {{{play_button_blinking}}}" role="button"><#
