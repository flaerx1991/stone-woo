...
class Sitemap extends Base_Widget {
	...
	private function render_sitemap_item( $sitemap_item, $query_args, $repeater_item_setting_key ) {
		...
		if ( ! $sitemap_item['hide_title'] ) {
			$title = empty( $title ) ? '' : sprintf( '<%s %s>%s</%1$s>',  $title_tag, $this->get_render_attribute_string( "{$title_tag}_{$item_type}" ), $title );
		} else {
			$title = '';
		}
		...
		if ( empty( $items_html ) ) {
			$empty_text = '' !== $settings['text_empty_list'] ? $settings['text_empty_list'] : __( 'This list is empty.', 'cmsmasters-elementor' );
