...
class Slider extends Base_Widget {
	...
	public function render_slides() {
		...
		foreach ( $settings['slides'] as $index => $slide ) {
			...
			if ( $slide['slide_bg_ken_burns'] && '1' === $settings['slider_per_view'] ) {
				$this->add_render_attribute(
					...
					array(
						...
						"cmsmasters-ken-burns-{$slide['ken_burns_direction']}",
					)
				);
			}
			...
			echo '<div class="elementor-widget-cmsmasters-slider__slide-container elementor-repeater-item-' . $slide['_id'] . ' ' . $container_class . '">' .
				'<div ' . $this->get_render_attribute_string( "slider-bg-{$index}" ) . '>';

			if ( 'video' === $slide['slide_bg_background'] ) {
				if ( $slide['slide_bg_video_link'] ) {
					...
					if ( $video_properties ) {
						if ( 'vimeo' !== $video_properties['provider'] ) {
							...
							echo Embed::get_embed_html( $slide['slide_bg_video_link'], $embed_params, $embed_options );
						} else {
							$embed_params = $this->get_embed_params( $slide, $video_properties );
						...
						echo '<video class="elementor-background-video-hosted elementor-html5-video" ' . $video_tag_attributes . '></video>';
					}
			...
			if ( $slide['slide_button_text'] && 'button' === $slide['slide_type_link'] ) {
				echo '<a class="elementor-widget-cmsmasters-slider__content-button' . esc_attr( $button_style ) . '" ' . $this->get_render_attribute_string( "slide-button-url-{$index}" ) . '>' . $slide['slide_button_text'] . '</a>';
