...
class Module extends Base_Module {
	...
	public function register_ajax_widget( AjaxWidgetModule $ajax_widget ) {
		$ajax_widget->add_handler( 'cmsmasters-mailchimp', array( $this, 'render_ajax_mailchimp' ) );
	}
	...
	public function render_ajax_mailchimp( $ajax_vars, Mailchimp $widget_obj, Ajax_Action_Handler $ajax_widget ) {
		...
		foreach ( $lists_id as $index => $list_id ) {
			...
			$url = $this->url() . $list_id . '/members/' . md5( strtolower( $fields['email'] ) );
	...
	protected function merge_fields( $ajax_vars ) {
		...
		if ( ! empty( $fields['mp-first-name'] ) ) {
			$merge_fields['FNAME'] = $fields['mp-first-name'];
		}

		if ( ! empty( $fields['mp-last-name'] ) ) {
			$merge_fields['LNAME'] = $fields['mp-last-name'];
		}

		if ( ! empty( $fields['phone'] ) ) {
			$merge_fields['PHONE'] = $fields['phone'];
		}
		...
		if ( ! empty( $fields['additionally'] ) || ! empty( $fields['city'] ) || ! empty( $fields['state'] ) || ! empty( $fields['zip'] ) || ! empty( $fields['country'] ) ) {
			$merge_fields['ADDRESS'] = array(
				'addr1' => ! empty( $fields['additionally'] ) ? $fields['additionally'] : '',
	...
	protected function message( $widget_obj ) {
		...
		$already_subscribed = empty( $settings['already_subscribed'] ) ? __( 'Given email address is already subscribed, thank you!', 'cmsmasters-elementor' ) : $settings['already_subscribed'];
		$unsubscribed = empty( $settings['unsubscribed'] ) ? __( 'You were successfully unsubscribed.', 'cmsmasters-elementor' ) : $settings['unsubscribed'];
		$not_subscribed = empty( $settings['not_subscribed'] ) ? __( 'Given email address is not subscribed.', 'cmsmasters-elementor' ) : $settings['not_subscribed'];
		...
		$success_optin_text = empty( $settings['success_optin_text'] ) ? __( 'Please go to your email address and confirm.', 'cmsmasters-elementor' ) : $settings['success_optin_text'];
		$update = empty( $settings['update'] ) ? __( 'Thank you, your records have been updated!', 'cmsmasters-elementor' ) : $settings['update'];
	...
	public function mailchimp_lists() {
		$lists = array();
		$url = $this->url() . '?fields=lists.id,lists.name&count=1000';
