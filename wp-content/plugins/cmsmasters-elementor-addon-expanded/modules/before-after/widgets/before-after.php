	...
	protected function section_overlay() {
		...
		foreach ( $overlay_items as $item ) {
			$this->start_controls_tab(
				$item . '_overlay_tab',
				array(
					'label' => esc_html__( ucfirst( $item ), 'cmsmasters-elementor' ),
					'condition' => array( 'overlay' => 'yes' ),
				)
			);
		...
		if ( $overlay ) {
			...
			foreach ( $overlay_items as $item ) {
				...
				if ( $item_label || $item_icon ) {
					echo '<div class="' . $this->get_widget_class() . '__overlay-' . $item . '-label_wrap">' .
						'<div class="' . $this->get_widget_class() . '__overlay-' . $item . '-label">';

					if ( $item_label ) {
						echo '<div class="' . $this->get_widget_class() . '__overlay-' . $item . '-label-text">' .
							$item_label .
						'</div>';
					}

					if ( $item_icon ) {
						echo '<div class="' . $this->get_widget_class() . '__overlay-' . $item . '-label-icon">';
							Icons_Manager::render_icon( $item_icon );
						echo '</div>';
					}
