...
abstract class Component_Base extends Sub_Controls_Stack {
	...
	public function __construct( $parent, $transform_element, $background_effect ) {
		...
		if ( 'common' === $this->parent->get_unique_name() ) {
			$exclusions = '{{WRAPPER}}';
			$exclusion_widget = '';
			...
			foreach ( $exclusion_widgets as $widget => $triger ) {
				...
				$exclusion_widget .= ', {{WRAPPER}}' . $widget . ' > .elementor-widget-container ' . $triger;
				$exclusion_hover_widget .= ', {{WRAPPER}}' . $widget . ' > .elementor-widget-container ' . $triger . ':hover';
			}

			$this->transform_wrapper = $exclusions . ' > .elementor-widget-container' . $exclusion_widget;

			$this->transform_hover_wrapper = $exclusions . ':hover > .elementor-widget-container' . $exclusion_hover_widget;
		} else {
	...
	protected function set_effect_vars() {
		...
		call_user_func( array( $this, "set_{$name}_vars" ) );
	}
