...
class Meta_Data extends Base_Widget {
		...
		foreach ( $groups as $group => $group_data ) {
			$repeater->add_control(
				...
				array(
					'label' => __( 'Type', 'cmsmasters-elementor' ),
					'type' => Controls_Manager::SELECT,
					'default' => array_keys( $group_data['options'] )[0],
	...
	protected function render_taxonomy() {
		...
		switch ( $settings['taxonomy_item_prefix_type'] ) {
			case 'icon':
				$args[ $settings['taxonomy_item_prefix_position'] ] = Utils::get_render_icon( $settings['taxonomy_item_prefix_icon'] );
	...
	protected function render_additional_text( $side ) {
		...
		$text_single = Utils::get_if_isset( $this->meta_field, $field_name );
		$text_plural = Utils::get_if_isset( $this->meta_field, "{$field_name}_plural" );
		...
		<div class="cmsmasters-meta-data">
			<div class="cmsmasters-meta-data__inner">
				...
				$this->loop_meta_fields( function() use ( $meta_fields ) {
					...
					echo '<div ' . ElementorUtils::render_html_attributes( $attrs ) . '>' .
