...
class Module extends Base_Module {
	...
	public static function render_time( $prefix, $settings, $postmeta_args = array() ) {
		...
		$time = Group_Control_Format_Time::get_render_format( $prefix, $settings, get_post_timestamp( null, 'time' ), $modified );
	...
	public static function render_date( $prefix, $settings, $postmeta_args = array() ) {
		...
		$date = Group_Control_Format_Date::get_render_format( $prefix, $settings, get_post_timestamp( null, 'date' ), $modified );

		static::render_postmeta(
			'date',
			function() use ( $date, $settings ) {
				if ( ! empty( $settings['date_link'] ) ) {
					echo '<a href="' . get_day_link( get_post_time( 'Y' ), get_post_time( 'm' ), get_post_time( 'j' ) ) . '">';
				}
