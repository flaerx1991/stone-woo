	...
	protected function register_controls() {
		...
		foreach ( $colors as $key => $label ) {
			...
			$this->add_group_control(
				...
				array(
					'name' => "login_button_bg_{$key}",
					'exclude' => array( 'color' ),
					'selector' => "{$selector} > a{$element}",
				)
			);

			$this->start_injection( array( 'of' => "login_button_bg_{$key}_background" ) );
		...
		foreach ( $colors as $key => $label ) {
			...
			$this->add_group_control(
				CmsmastersControls::BUTTON_BACKGROUND_GROUP,
				array(
					'name' => "logout_button_bg_{$key}",
					'exclude' => array( 'color' ),
					'selector' => "{$selector} > a{$element}",
				)
			);

			$this->start_injection( array( 'of' => "logout_button_bg_{$key}_background" ) );
		...
		if ( get_option( 'users_can_register' ) ) {
			...
			foreach ( $colors as $key => $label ) {
				...
				$this->add_group_control(
					CmsmastersControls::BUTTON_BACKGROUND_GROUP,
					array(
						'name' => "register_button_bg_{$key}",
						'exclude' => array( 'color' ),
						'selector' => "{$selector} > a{$element}",
					)
				);
	
				$this->start_injection( array( 'of' => "register_button_bg_{$key}_background" ) );
		...
		foreach ( $colors as $key => $label ) {
			...
			$this->add_group_control(
				CmsmastersControls::BUTTON_BACKGROUND_GROUP,
				array(
					'name' => "account_button_bg_{$key}",
					'exclude' => array( 'color' ),
					'selector' => "{$selector} > a{$element}",
				)
			);

			$this->start_injection( array( 'of' => "account_button_bg_{$key}_background" ) );
	...
	protected function render() {
		...
		if ( ! is_user_logged_in() || ( is_user_logged_in() && $is_editor ) ) {
			if ( $settings['show_login'] ) {
				...
				if ( '' !== $settings['prefix_text_login'] ) {
					echo '<span class="elementor-widget-cmsmasters-authorization-links__prefix">' . $settings['prefix_text_login'] . '</span>';
				}

					echo '<a ' . $this->get_render_attribute_string( 'login' ) . '>';

				if ( '' !== $settings['icon_login'] ) {
					Icons_Manager::render_icon( $settings['icon_login'] );
				}

				if ( '' !== $settings['login_text'] ) {
					echo '<span class="elementor-widget-cmsmasters-authorization-links__text">' . $settings['login_text'] . '</span>';
				}
		...
		if ( is_user_logged_in() || ( is_user_logged_in() && $is_editor ) ) {
			if ( $settings['show_logout'] ) {
				...
				$url = wp_logout_url( $settings['redirect_after_logout']['url'] );
				$link = "href='{$url}'";
				$target = '';
				$nofollow = '';
				...
				if ( '' !== $settings['prefix_text_logout'] ) {
					echo '<span class="elementor-widget-cmsmasters-authorization-links__prefix">' . $settings['prefix_text_logout'] . '</span>';
				}

					echo '<a ' . $link . $target . $nofollow . $this->get_render_attribute_string( 'logout' ) . '>';

				if ( '' !== $settings['icon_logout'] ) {
					Icons_Manager::render_icon( $settings['icon_logout'] );
				}

				if ( '' !== $settings['logout_text'] ) {
					echo '<span class="elementor-widget-cmsmasters-authorization-links__text">' . $settings['logout_text'] . '</span>';
				}
		...
		if ( ! is_user_logged_in() && get_option( 'users_can_register' ) || ( is_user_logged_in() && $is_editor && get_option( 'users_can_register' ) ) ) {
			if ( $settings['show_register'] ) {
				...
				if ( '' !== $settings['prefix_text_register'] ) {
					echo '<span class="elementor-widget-cmsmasters-authorization-links__prefix">' . $settings['prefix_text_register'] . '</span>';
				}

					echo '<a ' . $this->get_render_attribute_string( 'register' ) . '>';

				if ( '' !== $settings['icon_register'] ) {
					Icons_Manager::render_icon( $settings['icon_register'] );
				}

				if ( '' !== $settings['register_text'] ) {
					echo '<span class="elementor-widget-cmsmasters-authorization-links__text">' . $settings['register_text'] . '</span>';
				}
		...
		if ( is_user_logged_in() || ( is_user_logged_in() && $is_editor ) ) {
			...
			if ( '' !== $settings['account_page']['url'] ) {
				...
				if ( '' !== $settings['prefix_text_account'] ) {
					echo '<span class="elementor-widget-cmsmasters-authorization-links__prefix">' . $settings['prefix_text_account'] . '</span>';
				}

					echo '<a ' . $this->get_render_attribute_string( 'account' ) . '>';

				if ( '' !== $settings['icon_account'] ) {
					Icons_Manager::render_icon( $settings['icon_account'] );
				}

				if ( '' !== $settings['account_text'] ) {
					echo '<span class="elementor-widget-cmsmasters-authorization-links__text">' . $settings['account_text'] . '</span>';
				}
