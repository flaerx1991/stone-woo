...
class Toggles extends Base_Widget {
	...
	public function render() {
		...
		echo '<div class="' . $this->get_widget_class() . '__list" role="tablist">';
		...
		foreach ( $settings['toggles'] as $index => $item ) {
			...
			$this->add_render_attribute( $toggle_title_setting_key, array(
				...
				'data-tab' => $tab_count,
				'role' => 'tab',
				'aria-controls' => $this->get_widget_class() . '__content-' . $id_int . $tab_count,
				'aria-expanded' => 'false',
			) );
			...
			echo '<div class="' . $this->get_widget_class() . '__item"' . $custom_id . '>';
				echo '<' . $settings['title_html_tag'] . ' ' . $this->get_render_attribute_string( $toggle_title_setting_key ) . '>';
	...
	public function get_item_icon( $item ) {
		...
		echo '<span class="' . $this->get_widget_class() . '__item-icon" aria-hidden="true">';
	...
	public function get_trigger_icon() {
		...
		if ( isset( $settings['trigger_icon'] ) && ! empty( $settings['trigger_icon']['value'] ) ) {
			echo '<span class="' . $this->get_widget_class() . '__trigger" aria-hidden="true">';

				echo '<span class="' . $this->get_widget_class() . '__trigger-closed">';
					Icons_Manager::render_icon( $settings['trigger_icon'] );
				echo '</span>';
			...
			if ( isset( $settings['trigger_active_icon'] ) && ! empty( $settings['trigger_active_icon']['value'] ) ) {
				...
				echo '<span class="' . $this->get_widget_class() . '__trigger-opened">';
					Icons_Manager::render_icon( $trigger_active_icon );
				echo '</span>';
