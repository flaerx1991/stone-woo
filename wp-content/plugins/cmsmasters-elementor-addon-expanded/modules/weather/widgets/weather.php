...
class Weather extends Base_Widget {
	...
	public static function get_geo( $parameter = null ) {
		...
		if ( isset( $_COOKIE[ $cookie_name ] ) ) {
			...
			if ( ! in_array( 'error', self::geo_data() ) ) {
				$geo['region'] = $cookie_data[4];
				$geo['currency'] = $cookie_data[5];
				$geo['latitude'] = $cookie_data[6];
				$geo['longitude'] = $cookie_data[7];
				$geo['region_code'] = $cookie_data[8];
			}
		} else {
			$geo_data = self::geo_data();

			if ( ! in_array( 'error', $geo_data ) ) {
				$geo = array(
					'id' => $geo_data['ip'],
					'city' => $geo_data['city'],
	...
	final protected static function get_appID() {
