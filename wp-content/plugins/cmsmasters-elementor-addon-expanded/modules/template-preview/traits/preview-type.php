...
trait Preview_Type {
	...
	public static function get_singular_preview_type_options_choices( $woo = true ) {
		return array(
			'singular' => array(
				'label' => __( 'Singular', 'cmsmasters-elementor' ),
				'options' => static::get_singular_options_choices( $woo ),
			),
			'error_404' => __( '404 Error page', 'cmsmasters-elementor' ),
		);
	}

	public static function get_singular_options_choices( $woo = true ) {
		$post_type_args = ( $woo && WooModule::is_active() ) ? array( 'exclude' => '' ) : array();
	...
	public static function get_archive_preview_type_options_choices( $woo = true ) {
