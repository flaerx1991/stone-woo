		...
		$settings->add_section( 'cmsmasters', 'facebook_sdk', array(
			'callback' => function() {
				...
				echo '<p>' . sprintf( __( 'Facebook SDK lets you connect to your <a href="%s" target="_blank">dedicated application</a> so you can track the Facebook Widgets analytics on your site.', 'cmsmasters-elementor' ), 'https://developers.facebook.com/docs/apps/register/' ) .
					'<br>' .
					'<br>' .
					__( 'If you are using the Cmsmasters Comments Widget, you can add moderating options through your application. Note that this option will not work on local sites and on domains that don\'t have public access.', 'cmsmasters-elementor' ) . '</p>';
			},
			'fields' => array(
	...
	private function validate_sdk() {
		...
		if ( ! empty( $_POST['elementor_cmsmasters_facebook_app_id'] ) ) {
			...
			if ( is_wp_error( $response ) || 200 !== (int) wp_remote_retrieve_response_code( $response ) ) {
				$errors[] = __( 'Facebook App ID is not valid', 'cmsmasters-elementor' );
			}
		}
		...
		if ( ! empty( $errors ) ) {
			wp_die( $message, __( 'Facebook SDK', 'cmsmasters-elementor' ), array( 'back_link' => true ) );
		}
	}
}
