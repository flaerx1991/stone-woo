...
class Login extends Base {
	...
	protected function render() {
		...
		if ( 'yes' === $settings['label'] ) {
			$user_label = ( empty( $settings['user_label'] ) ? __( 'Username or Email Address', 'cmsmasters-elementor' ) : $settings['user_label'] );

			echo '<label ' . $this->get_render_attribute_string( 'user_label' ) . '>' . $user_label . '*</label>';
		}
		...
		if ( 'yes' === $settings['label'] ) {
			$password_label = ( empty( $settings['password_label'] ) ? __( 'Password', 'cmsmasters-elementor' ) : $settings['password_label'] );

			echo '<label ' . $this->get_render_attribute_string( 'password_label' ) . '>' . $password_label . '*</label>';
		}
		...
		if ( 'yes' === $settings['show_remember_me'] ) {
			...
			if ( ! empty( $settings['remember_me_text'] ) ) {
				$remember_me = $settings['remember_me_text'];
			}
			...
			echo '<div ' . $this->get_render_attribute_string( 'remember-me-wrapper' ) . '>
				...
				'</label>
					...
					$button_text = ( empty( $settings['button_text'] ) ? __( 'Log In', 'cmsmasters-elementor' ) : $settings['button_text'] );
		...
		$this->add_render_attribute(
			array(
				...
				'lost-pass' => array(
					...
					'href' => wp_lostpassword_url( $redirect_url ),
				),
