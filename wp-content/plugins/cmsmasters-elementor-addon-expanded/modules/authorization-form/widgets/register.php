...
class Register extends Base {
	...
	protected function render_registration_form() {
		...
		echo '<form class="elementor-widget-cmsmasters-register-form__register" method="post" action="' . wp_get_referer() . '">
			<div ' . $this->get_render_attribute_string( 'wrapper' ) . '>
				<div ' . $this->get_render_attribute_string( 'field-group' ) . '>';
		...
		if ( 'none' !== $settings['first_name'] ) {
			...
			if ( 'yes' === $settings['label'] ) {
				$fname_label = ( empty( $settings['first_name_label'] ) ? __( 'First Name', 'cmsmasters-elementor' ) : $settings['first_name_label'] );

				echo '<label ' . $this->get_render_attribute_string( 'label_fname' ) . '>' . $fname_label . $star_fname . '</label>';
			}
		...
		if ( 'none' !== $settings['last_name'] ) {
			...
			if ( 'yes' === $settings['label'] ) {
				$lname_label = ( empty( $settings['last_name_label'] ) ? __( 'Last Name', 'cmsmasters-elementor' ) : $settings['last_name_label'] );

				echo '<label ' . $this->get_render_attribute_string( 'label_lname' ) . '>' . $lname_label . $star_lname . '</label>';
			}
		...
		if ( 'yes' === $settings['label'] ) {
			$user_label = ( empty( $settings['user_label'] ) ? __( 'Username', 'cmsmasters-elementor' ) : $settings['user_label'] );

			echo '<label ' . $this->get_render_attribute_string( 'label_user' ) . '>' . $user_label . '*</label>';
		}
		...
		if ( 'none' !== $settings['website'] ) {
			...
			if ( $settings['label'] ) {
				$website_label = ( empty( $settings['website_label'] ) ? __( 'Website', 'cmsmasters-elementor' ) : $settings['website_label'] );

				echo '<label ' . $this->get_render_attribute_string( 'label_website' ) . '>' . $website_label . $star_website . '</label>';
			}
				...
				$button_text = ( empty( $settings['button_text'] ) ? __( 'Register', 'cmsmasters-elementor' ) : $settings['button_text'] );
		...
		if ( 'yes' === $settings['login'] ) {
			$login_text = ( empty( $settings['login_text'] ) ? __( 'Login', 'cmsmasters-elementor' ) : $settings['login_text'] );

			echo '<a ' . $this->get_render_attribute_string( 'login-link' ) . '">' . $login_text . '</a>';
		}

				echo '</div>
			</div>';

			$user_short = ( '' === $settings['user_short'] ? __( 'The username is too short. At least 4 characters are required.', 'cmsmasters-elementor' ) : $settings['user_short'] );
			$user_exists = ( '' === $settings['user_exists'] ? __( 'Sorry, that username already exists. Please try another one.', 'cmsmasters-elementor' ) : $settings['user_exists'] );
			$user_novalid = ( '' === $settings['user_novalid'] ? __( 'Sorry, the username you entered is not valid. Please use only letters (a-z), numbers and periods.', 'cmsmasters-elementor' ) : $settings['user_novalid'] );
			$email_novalid = ( '' === $settings['email_novalid'] ? __( 'Invalid email address. Please enter a valid address. Example: someone@example.com', 'cmsmasters-elementor' ) : $settings['email_novalid'] );
			$email_exists = ( '' === $settings['email_exists'] ? __( 'That email is already in use. Please try another one.', 'cmsmasters-elementor' ) : $settings['email_exists'] );

			wp_nonce_field( 'register', 'register-nonce' );
			echo '<input type="hidden" name="role" value="' . $settings['role'] . '">
			<input type="hidden" name="type-redirect" value="' . $settings['redirect_register'] . '">
	...
	private function form_fields_render_attributes() {
		...
		if ( 'yes' === $settings['placeholder'] && ! empty( $settings['first_name_placeholder'] ) ) {
			$first_name_placeholder = $settings['first_name_placeholder'];
		...
		if ( 'yes' === $settings['placeholder'] && ! empty( $settings['last_name_placeholder'] ) ) {
			$last_name_placeholder = $settings['last_name_placeholder'];
		...
		if ( 'yes' === $settings['placeholder'] && ! empty( $settings['website_placeholder'] ) ) {
			$website_placeholder = $settings['website_placeholder'];
		...
		$this->add_render_attribute(
			array(
				...
				'login-link' => array(
					'href' => ( '' === $settings['login_url'] ? wp_login_url() : $settings['login_url'] ),
					'class' => array(
						'elementor-widget-cmsmasters-register-form__field-group',
	...
	protected function render_correct_valid() {
		...
		if ( 'email' === $settings['redirect_register'] ) {
			...
			$user_valid = ( '' === $settings['user_valid'] ? __( 'Registration successful. The password has been sent to your email, please check the Inbox and Spam folder.', 'cmsmasters-elementor' ) : $settings['user_valid'] );

			echo '<div class="elementor-widget-cmsmasters-register-form__valid' . $backend_class . '">' . $user_valid . '</div>';
		}
	}
	...
	protected function render_no_valid_on_backend() {
		...
		$user_short = ( '' === $settings['user_short'] ? __( 'The username is too short. At least 4 characters are required.', 'cmsmasters-elementor' ) : $settings['user_short'] );
		$user_exists = ( '' === $settings['user_exists'] ? __( 'Sorry, that username already exists. Please try another one.', 'cmsmasters-elementor' ) : $settings['user_exists'] );
		$user_novalid = ( '' === $settings['user_novalid'] ? __( 'Please use only letters (a-z), numbers and periods.', 'cmsmasters-elementor' ) : $settings['user_novalid'] );
		$email_novalid = ( '' === $settings['email_novalid'] ? __( 'Invalid email address. Please enter a valid address. Example: someone@example.com', 'cmsmasters-elementor' ) : $settings['email_novalid'] );
		$email_exists = ( '' === $settings['email_exists'] ? __( 'That email is already in use. Please try another one.', 'cmsmasters-elementor' ) : $settings['email_exists'] );
