		...
		foreach ( $colors as $key => $label ) {
			...
			$this->add_group_control(
				...
				array(
					'name' => "button_bg_{$key}",
					'exclude' => array( 'color' ),
					'selector' => "{{WRAPPER}} .elementor-widget-cmsmasters-{$form_name}-form__button{$element}",
				)
			);
	...
	protected function render_logged_message( $logout_redirect ) {
		...
		if ( 'yes' === $settings['show_logged_in_message'] ) {
			...
			if ( empty( $settings['text_logged_in_message'] ) ) {
				...
				$text_logged = $settings['text_logged_in_message'];
			}

			if ( empty( $settings['link_logged_in_message'] ) ) {
				$link_text_logged = __( 'Logout', 'cmsmasters-elementor' );
			} else {
				$link_text_logged = $settings['link_logged_in_message'];
			}

			echo '<div class="elementor-widget-cmsmasters-' . $form_name . '-form__' . $form_name . ' elementor-widget-cmsmasters-' . $form_name . '-form__logged-in-message' . $backend_class . '">' .
				/* translators: Addon 'Logged in message' info. %1$s: Current user name, %2$s: Page where the transition will be completed */
				sprintf( '' . $text_logged . ' %1$s (<a href="%2$s">' . $link_text_logged . '</a>)', $current_user->display_name, wp_logout_url( $logout_redirect ) ) .
			'</div>';
		}
	}
