...
class Site_Logo extends Base_Widget {
	...
	protected function register_controls() {
		...
		$this->add_responsive_control(
			...
			array(
				...
				'prefix_class' => 'cmsmasters-logo%s-alignment-',
			)
		);
		...
		$site_logo_subtitle_text = Utils::get_kit_option( 'cmsmasters_logo_subtitle_text', '' );
	...
	public function get_logo_wrapper() {
		...
		if ( 'icon' === $widget_image_type && ! empty( $settings['logo_icon'] ) ) {
			echo '<span class="elementor-widget-cmsmasters-site-logo__icon">';
				Icons_Manager::render_icon( $settings['logo_icon'] );
			echo '</span>';
		}
	...
	public function get_logo_image() {
		...
		if ( empty( $logo_out ) ) {
			$logo_out = '<img' .
				' class="cmsmasters-lazyload-disabled elementor-widget-cmsmasters-site-logo__img"' .
	...
	public function get_logo_img( $atts = array(), $type = 'normal' ) {
		...
		if ( 'retina' === $type ) {
			...
			$img_atts[] = 'class="cmsmasters-lazyload-disabled ' . esc_attr( "{$parent_class}__retina-img {$parent_class}-{$state}" ) . '"';
		} else {
			$img_atts[] = 'class="cmsmasters-lazyload-disabled ' . esc_attr( "{$parent_class}__img {$parent_class}-{$state}" ) . '"';
		}
	...
	public function get_logo_subtitle() {
		...
		if ( '' !== $logo_subtitle ) {
			$subtitle = esc_html( $logo_subtitle );
		}

		if ( '' === $logo_subtitle && '' !== $site_logo_subtitle_text ) {
			$subtitle = esc_html( $site_logo_subtitle_text );
		}
