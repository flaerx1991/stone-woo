...
class Off_Canvas extends Base_Widget {
	...
	protected function register_controls() {
		...
		$repeater->add_control(
			...
			array(
				...
				'default' => Utils::get_kit_option( 'cmsmasters_logo_type', 'image' ),
				'label_block' => false,
		...
		$site_logo_title_text = Utils::get_kit_option( 'cmsmasters_logo_title_text', '' );
		...
		if ( ! array_key_exists( 'no_template', $this->get_page_template_options( 'section' ) ) ) {
			$repeater->add_control(
				'saved_section',
				array(
		...
		$this->add_group_control(
			...
			array(
				...
				'label' => __( 'Typography', 'cmsmasters-elementor' ),
				'selector' => '.cmsmasters-offcanvas-content-{{ID}} ' . $widget_selector . '__body ' . $widget_selector . '__custom-container-cont-inner > *:not(' . $widget_selector . '__site-logo),
		...
		$this->add_control(
			...
			array(
				...
				'default' => '',
				'selectors' => array(
					'.cmsmasters-offcanvas-content-{{ID}} ' . $widget_selector . '__menu-container,
					.cmsmasters-offcanvas-content-{{ID}} ' . $widget_selector . '__menu-container a,
					.cmsmasters-offcanvas-content-{{ID}} ' . $widget_selector . '__body ' . $widget_selector . '__custom-container-cont-inner > div:not(' . $widget_selector . '__site-logo),
		...
		$this->add_group_control(
			...
			array(
				...
				'label' => __( 'Typography', 'cmsmasters-elementor' ),
				'selector' => '.cmsmasters-offcanvas-content-{{ID}} ' . $widget_selector . '__body ' . $widget_selector . '__custom-container-cont-inner > *:not(' . $widget_selector . '__site-logo) a,
		...
		$this->add_control(
			...
			array(
				...
				'default' => '',
				'selectors' => array(
					'.cmsmasters-offcanvas-content-{{ID}} ' . $widget_selector . '__body ' . $widget_selector . '__custom-container-cont-inner > *:not(' . $widget_selector . '__site-logo) a'  => 'color: {{VALUE}}',
		...
		$this->add_control(
			...
			array(
				...
				'default' => '',
				'selectors' => array(
					'.cmsmasters-offcanvas-content-{{ID}} ' . $widget_selector . '__body ' . $widget_selector . '__custom-container-cont-inner > *:not(' . $widget_selector . '__site-logo) a:hover,
		...
		$this->start_injection( array( 'of' => 'trigger_bg_background' ) );
		...
		$this->start_injection( array( 'of' => 'trigger_bg_hover_background' ) );
		...
		$this->start_injection( array( 'of' => 'trigger_bg_active_background' ) );
		...
		$this->add_control(
			...
			array(
				...
				'selectors' => array(
					...
					.cmsmasters-offcanvas-content-{{ID}} ' . $widget_selector . '__body ' . $widget_selector . '__menu-inner li.current-menu-item > a:hover' => 'color: {{VALUE}}',
				),
			)
		);
		...
		$this->add_control(
			...
			array(
				...
				'selectors' => array(
					...
					.cmsmasters-offcanvas-content-{{ID}} ' . $widget_selector . '__body ' . $widget_selector . '__menu-inner ul > li.current-menu-item > a:hover' => 'color: {{VALUE}}',
				),
			)
		);
	...
	public function get_template_ids() {
		...
		foreach ( $content_block as $item ) {
			...
			if ( ! Utils::is_publish( $template_id ) ) {
				continue;
			}
	...
	public function get_trigger() {
		...
		$this->add_render_attribute( 'cmsmasters-offcanvas-trigger-button', 'class', 'elementor-widget-cmsmasters-offcanvas__trigger' );
	...
	public function get_content() {
		...
		if ( count( $settings['content_block'] ) === $this->box_to_down_count ) {
			...
			echo '<div class="elementor-widget-cmsmasters-offcanvas__body">' .
				'<div class="elementor-widget-cmsmasters-offcanvas__body-container' . $content_block_all_down . '">';
		...
		foreach ( $settings['content_block'] as $item ) {
			...
			echo '<div class="elementor-widget-cmsmasters-offcanvas__custom-container ' .
				...
				' cmsmasters-box-down-' . $item['box_to_down'] . '">';

				echo '<div class="elementor-widget-cmsmasters-offcanvas__custom-container-cont">' .
					'<div class="elementor-widget-cmsmasters-offcanvas__custom-container-cont-inner' . $item_bg_enable . '">';
				...
				case 'widget':
					...
					if ( '-1' === $template_id || ! $template_id ) {
						if ( is_admin() ) {
							...
							Utils::render_alert( sprintf( esc_html__( 'Please choose your %s template!', 'cmsmasters-elementor' ), $template_name ) );
						}
	...
	public function get_logo_wrapper( $item ) {
		$site_logo_type = Utils::get_kit_option( 'cmsmasters_logo_type', 'image' );
	...
	public function get_img_logo_id( $item, $state = 'main' ) {
		...
		$site_logo_type = Utils::get_kit_option( 'cmsmasters_logo_type', 'image' );
		$site_logo = Utils::get_kit_option( "cmsmasters_logo_image{$suffix}", array( 'id' => '' ) );
	...
	public function get_img_logo_retina_id( $item, $state = 'main' ) {
		...
		$site_logo_type = Utils::get_kit_option( 'cmsmasters_logo_type', 'image' );
		$site_logo = Utils::get_kit_option( "cmsmasters_logo_image{$suffix}", array( 'id' => '' ) );
		$site_logo_retina_toggle = Utils::get_kit_option( 'cmsmasters_logo_retina_toggle' );
		$site_logo_retina_toggle = ( 'main' !== $state ? 'yes' : $site_logo_retina_toggle );
		$site_logo_second_toggle = Utils::get_kit_option( 'cmsmasters_logo_second_toggle' );
		$site_logo_second_toggle = ( 'main' !== $state ? $site_logo_second_toggle : 'yes' );
		$site_logo_retina = Utils::get_kit_option( "cmsmasters_logo_retina_image{$suffix}", array( 'id' => '' ) );
	...
	public function get_widget_template( $template_id, $type ) {
		if ( ! $template_id ) {
			if ( is_admin() ) {
				...
				Utils::render_alert( esc_html( $message ) );
			}
	...
	public function added_span_in_link( $item, $depth ) {
		...
		if ( ! empty( $indicator['value'] ) ) {
			$new_title .= '<span class="' . $this->get_widget_class() . '__arrow cmsmasters-indicator-position-' . $indicator_position . '">' .
				Utils::get_render_icon( $indicator ) .
			'</span>';
		}
	...
	public function get_close() {
		...
		echo '<div ' . $this->get_render_attribute_string( 'cmsmasters-offcanvas-close-container' ) . '>' .
			'<div class="elementor-widget-cmsmasters-offcanvas__close">';
