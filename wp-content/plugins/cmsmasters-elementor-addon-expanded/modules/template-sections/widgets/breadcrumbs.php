...
class Breadcrumbs extends Breadcrumbs_Base {
	...
	public function get_breadcrumbs() {
		...
		if (
			...
			if ( is_single() ) {
				...
				echo '<span>' . __( 'Search results for', 'cmsmasters-elementor' ) . ": '" . esc_html( get_search_query() ) . "'</span>";
			} elseif ( is_page() && ! $post->post_parent ) {
				echo '<span>' . get_the_title( get_the_ID(), false ) . '</span>';
			} elseif ( is_page() && $post->post_parent ) {
				$this->get_page_type();
			} elseif ( is_author() ) {
				...
				if ( isset( $breadcrumbs_404 ) && '' !== $breadcrumbs_404 ) {
					echo $breadcrumbs_404;
				} else {
					echo __( 'Error 404: Page not found', 'cmsmasters-elementor' );
				}

				echo '</span>';
			} else {
				echo '<span>' . __( 'No breadcrumbs', 'cmsmasters-elementor' ) . '</span>';
			}
		}
	}
	...
	public function get_separator() {
		...
		if ( 'icon' === $separator_type ) {
			...
			if ( '' !== $icon_separator['value'] ) {
				Icons_Manager::render_icon( $icon_separator );
			} else {
	...
	public function get_prefix_content( $style = '' ) {
		...
		if ( 'archive_' === $style ) {
			$static_prefix = 'Archives for:';
		} elseif ( 'search_' === $style ) {
			$static_prefix = 'You searched for:';
		} else {
			$static_prefix = 'Browse:';
		}
	...
	public function get_page_type() {
		...
		echo '<span>' .
			get_the_title( get_the_ID(), false ) .
		'</span>';
	}
