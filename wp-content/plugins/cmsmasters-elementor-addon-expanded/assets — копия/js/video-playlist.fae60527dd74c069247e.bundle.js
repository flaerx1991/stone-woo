...
class VideoPlayList extends _handler.default {
  ...
  getDefaultSettings() {
    ...
    const selectors = {
      ...
      canvasInner: `.${classes.widget}__canvas_inner`,
  ...
  getDefaultElements() {
    ...
    const elements = {
      ...
      $canvasInner: this.findElement(selectors.canvasInner),
  ...
  initPlaylistCb(YT) {
    ...
    $container.on('click', selectors.item, event => {
      this.onPlay(jQuery(event.currentTarget));
    });
    const $firstItem = this.elements.$item.eq(0);
    this.elements.$canvasCover.on('click', () => {
  ...
  onPlay(item) {
    ...
    this.elements.$container.find(this.elements.$canvasInner).parent().addClass(classes.canvasActive);
  ...
  switchVideo($el) {
    ...
    const $canvasInner = $el.closest(this.elements.$container).find(this.elements.$canvasOverlay);
    ...
    const currentPlayer = $canvasInner.data('player');
    if ($canvas.hasClass(classes.canvasActive) && $itemStatus.hasClass(classes.statusPlaying) && currentPlayer) {
      this.pauseCurrentPlayer(currentPlayer, $canvasInner.data('provider'));
    }
  ...
  stopVideo() {
    ...
    const $canvas = this.elements.$canvas;
    const $canvasInner = this.elements.$canvasInner;
    if ($canvas.hasClass(classes.canvasActive)) {
      $canvas.removeClass(classes.canvasActive);
      this.pauseCurrentPlayer($canvasInner.data('player'), $canvasInner.data('provider'));
    }
  }
  initPerfectScroll() {
...
//# sourceMappingURL=video-playlist.fae60527dd74c069247e.bundle.js.map