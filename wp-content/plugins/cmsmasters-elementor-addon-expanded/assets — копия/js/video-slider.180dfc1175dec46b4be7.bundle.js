...
class VideoSlider extends _handler.default {
  ...
  bindEvents() {
    this.elements.$arrowButton.on('click', () => {
  ...
  async onInit() {
    ...
    setTimeout(() => {
      this.elements.$widgetWrap.find('.elementor-repeater-item').find(selectors.imageOverlay).on('click', () => {
        this.handleVideo(jQuery(event.currentTarget));
      });
      this.elements.$widgetWrap.find('.elementor-repeater-item').find(selectors.stopVideoButton).on('click', () => {
...
//# sourceMappingURL=video-slider.180dfc1175dec46b4be7.bundle.js.map