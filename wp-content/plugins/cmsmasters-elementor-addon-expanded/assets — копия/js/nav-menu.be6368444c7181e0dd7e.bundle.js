...
class MenuWidget extends _handler.default {
  ...
  checkPreventDefault(event) {
    ...
    let $clickedItem = jQuery(event.target);
    if (!$clickedItem.attr('href')) {
      $clickedItem = jQuery(event.target).closest('a');
    }
    const itemHasHref = $clickedItem.attr('href');
    const itemHasPlug = '#' === $clickedItem.attr('href');
    const itemHasArrow = 1 === $clickedItem.find(selectors.menuArrow).length;
    const targetArrow = 1 === jQuery(event.target).closest(selectors.menuArrow).length;
    if ($clickedItem.parent().hasClass(classes.itemHasChildren)) {
      if ((!itemHasHref || itemHasPlug || itemHasArrow) && targetArrow || !itemHasArrow) {
        event.preventDefault();
      }
      const $menuContainer = this.elements.$menuContainer;
      if (!$menuContainer.hasClass(classes.navMenuOpenLink) && ($menuContainer.hasClass(`${classes.verticalType}-toggle`) || $menuContainer.hasClass(`${classes.verticalType}-accordion`))) {
        event.preventDefault();
      }
    }
  }
  ...
  getParentItem(event) {
    ...
    if ($clickedItem.hasClass(classes.menuItem)) {
      ...
      $parentItem = $clickedItem.offsetParent(classes.dropdownItem);
    }
    return $parentItem;
  }
  ...
  onParentClick(event) {
    ...
    if ($nextItem.is(`ul, ${selectors.megaMenuContainer}`) && !$nextItem.is(':visible')) {
...
//# sourceMappingURL=nav-menu.be6368444c7181e0dd7e.bundle.js.map