...
class Pagination extends elementorModules.ViewModule {
  ...
  onSuccess() {
    ...
    if (this.isInfiniteScroll()) {
      setTimeout(() => {
        ...
        if (this.waypoint) {
          this.waypoint.element.trigger('resize.waypoints');
        }
      }, 150);
    }
  }
  ...
  initInfiniteScroll() {
    ...
    this.widget.$element.imagesLoaded(() => {
      if (this.waypoint) {
        this.waypoint.destroy();
      }
      this.waypoint = new Waypoint({
        element: this.widget.elements.$variable,
        handler: direction => {
          if ('down' !== direction) {
            return;
          }
...
//# sourceMappingURL=b5e93cab99c1fc9fff75.bundle.js.map