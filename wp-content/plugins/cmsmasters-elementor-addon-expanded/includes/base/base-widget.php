...
abstract class Base_Widget extends Widget_Base {
	...
	/**
	 * Get widget css config.
	 *
	 * @since 1.6.5
	 *
	 * @param string $widget_name Widget name.
	 *
	 * @return array Widget css config.
	 */
	public function get_widget_css_config( $widget_name ) {
		...
		$file_url = CmsmastersElementorPlugin::instance()->frontend->get_frontend_file_url( $file_name, $has_custom_breakpoints );
		...
		$file_path = CmsmastersElementorPlugin::instance()->frontend->get_frontend_file_path( $file_name, $has_custom_breakpoints );
	...
	/**
	 * Get responsive widgets config.
	 *
	 * @since 1.6.5
	 *
	 * @return array Responsive widgets config.
	 */
	public function get_responsive_widgets_config() {
		$responsive_widgets_data_manager = $this->get_responsive_widgets_data_manager();

		return [
			'key' => 'cmsmasters-responsive-widgets',
			'version' => CMSMASTERS_ELEMENTOR_VERSION,
			'file_path' => CMSMASTERS_ELEMENTOR_ASSETS_PATH . $responsive_widgets_data_manager::RESPONSIVE_WIDGETS_FILE_PATH,
		];
	}
	...
	protected function is_custom_breakpoints_widget() {
		...
		if ( $has_custom_breakpoints ) {
			...
			$current_widget_name = $this->get_group_name();
	...
	protected function render_by_mode() {
		if ( 'enable' === $this->lazyload_widget_get_status() ) {
			...
			if ( ! empty( $template_ids ) ) {
				CmsmastersElementorPlugin::instance()->frontend->lazyload_widget_enqueue_template_css( $template_ids, $this->get_id() );
			}

			echo '<div ' . $this->get_render_attribute_string( 'lazyload_widget_settings' ) . '>';

				if ( ! empty( $preloader_icon['value'] ) ) {
					echo '<span class="cmsmasters-lazyload-widget-preloader-icon">' .
