...
"use strict";
(self["webpackChunkcmsmasters_elementor_addon"] = self["webpackChunkcmsmasters_elementor_addon"] || []).push([["theme-my-account"],{
...
/***/ "../modules/theme/assets/dev/js/frontend/base/select2.js":
/*!***************************************************************!*\
  !*** ../modules/theme/assets/dev/js/frontend/base/select2.js ***!
  \***************************************************************/
...
/***/ "../modules/theme/assets/dev/js/frontend/widgets/theme-my-account.js":
/*!***************************************************************************!*\
  !*** ../modules/theme/assets/dev/js/frontend/widgets/theme-my-account.js ***!
  \***************************************************************************/
/***/ ((__unused_webpack_module, exports, __webpack_require__) => {
...
exports["default"] = void 0;
var _select = _interopRequireDefault(__webpack_require__(/*! ../base/select2 */ "../modules/theme/assets/dev/js/frontend/base/select2.js"));
class ThemeMyAccount extends _select.default {
  getDefaultSettings() {
    const baseClass = 'elementor-widget-cmsmasters-theme-my-account';
    const selectors = {
      address: 'address',
      ...
      quotesOrder: `.${baseClass}__order`,
      quotesOrderButton: `.${baseClass}__order-details-button`,
      quotesOrderTotalsWrap: `.${baseClass}__order-totals-wrap`,
      popup: `.${baseClass}__popup-edit-address`,
      popupInner: `.${baseClass}__popup-edit-address-inner`,
      popupCloseIcon: `.my-account-popup-edit-address-close`,
  ...
  getDefaultElements() {
    ...
    const elements = {
      ...
      $quotesOrderButton: this.$element.find(selectors.quotesOrderButton),
      $popupCloseIcon: this.$element.find(selectors.popupCloseIcon),
      $edit: this.$element.find(selectors.edit)
    };
    return elements;
  }
  bindEvents() {
    super.bindEvents();
    this.elements.$quotesOrderButton.on('click', this.quotesOrderDetailsOpen.bind(this));
    this.elements.$edit.on('click', this.popupAddressClick.bind(this));
    this.elements.$popupCloseIcon.on('click', this.popupCloseIconClick.bind(this));
  }
  onInit() {
  ...
  editorInitTabs() {
    this.elements.$allPageElements.each((index, element) => {
      ...
      $linksToThisPage.on('click', event => {
  ...
  quotesOrderDetailsOpen(event) {
    const {
      selectors
    } = this.getSettings();
    const $this = jQuery(event.currentTarget);
    const $quotesOrder = $this.closest(selectors.quotesOrder);
    const $quotesOrderTotalsWrap = $quotesOrder.find(selectors.quotesOrderTotalsWrap);
    $this.toggleClass('active');
    if ($this.hasClass('active')) {
      $quotesOrder.addClass('active');
      $quotesOrderTotalsWrap.slideDown(200, 'linear');
    } else {
      $quotesOrder.removeClass('active');
      $quotesOrderTotalsWrap.slideUp(200, 'linear');
    }
  }
  popupCloseIconClick(event) {
    const {
      selectors
    } = this.getSettings();
    const $this = jQuery(event.currentTarget);
    $this.closest(selectors.popup).removeClass('visible').addClass('hidden').find(selectors.popupInner).removeClass('visible').addClass('hidden');
    $this.closest('html').css('overflow-y', '');
  }
  popupAddressClick(event) {
    const {
      selectors
    } = this.getSettings();
    event.preventDefault();
    ...
    const type = link.match(/\/([^\/]+)\/$/)[1];
    $this.closest(selectors.woocommerce).find(`${selectors.popup}[type="${type}"]`).removeClass('hidden').addClass('visible').find(selectors.popupInner).removeClass('hidden').addClass('visible');
    $this.closest('html').css('overflow-y', 'hidden');
  }
}
exports["default"] = ThemeMyAccount;

/***/ })

}]);
//# sourceMappingURL=theme-my-account.205035e739e22c83e896.bundle.js.map