...
class AudioPlayList extends _handler.default {
  ...
  bindEvents() {
    this.elements.$playButton.on('click', this.player.bind(this));
    ...
    this.elements.$loopButton.on('click', () => this.getLoopClass());
    this.elements.$prevButton.on('click', () => this.prev());
    this.elements.$nextButton.on('click', () => this.next());
    this.elements.$backwardButton.on('click', () => this.rewind(event));
    this.elements.$forwardButton.on('click', () => this.rewind(event));
    this.elements.$track.on('click', () => this.clickTrack(event));
    this.elements.$listButton.on('click', () => this.togglePlaylist(event));
    this.elements.$advancedIcon.on('click', () => this.openAdvanced(event));
    jQuery(document).on('click', () => this.closeAdvanced(event));
    this.elements.$speedButton.on('click', () => this.changeSpeed(event));
    this.elements.$progressInner.on('mousemove', this.viewCurrentTime.bind(this));
    this.elements.$shuffleButton.on('click', () => this.activeShuffle(event));
  }
  onPlay() {
    const {
      classes
    } = this.getSettings();
    this.elements.$speedRate.on('click', () => {
  ...
  volumeChange() {
    ...
    volumeButton.on('click', this.volumeToggle.bind(this));
  }
  volumeToggle() {
    const audioTag = this.elements.$audioTag.get(0);
    const volumeProgress = this.elements.$volumeProgress;
...
//# sourceMappingURL=audio-playlist.d126d553b79e75819a4d.bundle.js.map