...
class Merlin {
	...
	protected function welcome() {
		...
		<div class="merlin__content--transition">
			...
			<h1><?php echo esc_html( sprintf( $header, $this->theme_name ) ); ?></h1>

			<p><?php echo esc_html( sprintf( $paragraph, $this->theme_name ) ); ?></p>
	...
	protected function license() {
		...
		<div class="merlin__content--transition">
			...
			<p id="license-text"><?php echo esc_html( sprintf( $paragraph, $this->theme_name ) ); ?></p>
			...
			<?php if ( $count ) : ?>
				...
				<?php if ( ! empty( $recommended_plugins ) ) : ?>
					<?php foreach ( $recommended_plugins as $slug => $plugin ) : ?>
						<li data-slug="<?php echo esc_attr( $slug ); ?>">
							<input type="checkbox" name="default_plugins[<?php echo esc_attr( $slug ); ?>]" class="checkbox" id="default_plugins_<?php echo esc_attr( $slug ); ?>" value="0">
		...
		if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
			...
			if ( false === $license_data->success ) {
				...
				if ( 'valid' === $license_data->license ) {
					$message = sprintf( esc_html( $success_message ), $this->theme_name );
					$success = true;
	...
	public function after_content_import_setup() {
		...
		if ( ! empty( $homepage->post ) && ! empty( $homepage->post->ID ) ) {
			...
			$this->logger->debug( __( 'The home page was set', 'merlin-wp' ), array( 'homepage_id' => $homepage->post->ID ) );
		}
		...
		if ( ! empty( $blogpage->post ) && ! empty( $blogpage->post->ID ) ) {
			...
			$this->logger->debug( __( 'The blog page was set', 'merlin-wp' ), array( 'blog_page_id' => $blogpage->post->ID ) );
		}
	}
	...
	public function before_content_import_setup() {
		...
		$hello_world = new \WP_Query( array(
			'post_type' => 'post',
			'title' => 'Hello world!',
			'post_status' => 'any',
			'posts_per_page' => 1,
