...
class Woo_Product_Filter {
	...
	public static function get_import_status( $default = 'done' ) {
		return get_option( 'cmsmasters_stereo-bank_' . Utils::get_demo() . '_woo_product_filter_import', $default );
	}
	...
	public static function set_import_status( $status = 'pending' ) {
		...
		update_option( 'cmsmasters_stereo-bank_' . Utils::get_demo() . '_woo_product_filter_import', $status );
	}
	...
	protected function import_filters() {
		$response = API_Requests::post_request( 'get-woo-product-filter', array( 'demo' => Utils::get_demo() ) );
