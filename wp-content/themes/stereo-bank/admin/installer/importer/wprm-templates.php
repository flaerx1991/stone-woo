...
class WPRM_Templates {
	...
	public static function get_import_status( $default = 'done' ) {
		return get_option( 'cmsmasters_stereo-bank_' . Utils::get_demo() . '_wprm_templates_import', $default );
	}
	...
	public static function set_import_status( $status = 'pending' ) {
		...
		update_option( 'cmsmasters_stereo-bank_' . Utils::get_demo() . '_wprm_templates_import', $status );
	}
	...
	protected function import_templates() {
		$response = API_Requests::post_request( 'get-wprm-templates', array( 'demo' => Utils::get_demo() ) );
