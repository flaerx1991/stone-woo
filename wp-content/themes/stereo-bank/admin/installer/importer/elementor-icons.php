...
class Elementor_Icons {
	...
	public static function set_import_status( $status = 'pending' ) {
		...
		if ( 'done' === get_option( "cmsmasters_stereo-bank_{$demo}_elementor_icons_import" ) ) {
			return;
		}

		update_option( "cmsmasters_stereo-bank_{$demo}_elementor_icons_import", $status );
	}
	...
	public function get_local_icons() {
		...
		if ( 'pending' !== get_option( "cmsmasters_stereo-bank_{$demo}_elementor_icons_import", 'done' ) ) {
			return;
		}

		$response = API_Requests::post_request( 'get-elementor-icons', array( 'demo' => Utils::get_demo() ) );
		...
		if ( empty( $data ) || empty( $data['icons_path'] ) ) {
			return;
		}

		$file_path = File_Manager::download_temp_file( $data['icons_path'], 'icons-' . uniqid() . '.zip' );

		$this->import_icons( basename( $file_path ), $file_path );

		@unlink( $file_path ); // phpcs:ignore WordPress.PHP.NoSilencedErrors.Discouraged

		update_option( "cmsmasters_stereo-bank_{$demo}_elementor_icons_import", 'done' );
	}
