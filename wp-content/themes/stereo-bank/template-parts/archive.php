...
use StereoBankSpace\TemplateFunctions\Post_Elements;
...
if ( is_search() && ! have_posts() ) {
	echo '<div class="' . $parent_class . '__no-posts">' .
		'<h3 class="' . $parent_class . '__no-posts-title">' . esc_html__( 'Nothing found', 'stereo-bank' ) . '</h3>' .
		'<div class="' . $parent_class . '__no-posts-description">' .
			'<p>' . esc_html__( 'Sorry, no posts matched your criteria. Please try another search.', 'stereo-bank' ) . '</p>' .
			'<p>' . esc_html__( 'You might want to consider some of our suggestions to get better results:', 'stereo-bank' ) . '</p>' .
			'<ul>' .
				'<li>' . esc_html__( 'Check your spelling.', 'stereo-bank' ) . '</li>' .
				'<li>' . esc_html__( 'Try a similar keyword, for example: tablet instead of laptop.', 'stereo-bank' ) . '</li>' .
				'<li>' . esc_html__( 'Try using more than one keyword.', 'stereo-bank' ) . '</li>' .
			'</ul>' .
		'</div>' .
	'</div>';
}
