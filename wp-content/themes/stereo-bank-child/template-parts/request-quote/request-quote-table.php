...
/*if ( is_user_logged_in() ) {
	...
	$is_allow_role = false;
	$stone_roles = get_field('user_role_discounts', 'options');
	if($stone_roles) {
		foreach($stone_roles as  $stone_role) {
			if( $current_user_role == $stone_role["user_role"]) $is_allow_role = true;
		}
	}
}*/
				...
				<table class="shop_table cart  <?php echo esc_attr( $responsive_class ); ?>" id="yith-ywrq-table-list">
					...
					<tr>
						...
						<th class="product-name" colspan="2">
							<?php esc_html_e( 'Product', 'stone' ); ?>
						</th>
						<th class="product-uom"><?php esc_html_e( 'UOM', 'stone' ); ?></th>
						<?php //if($is_allow_role) : ?>
							<!--th class="product-price"><?php //esc_html_e( 'Price', 'stone' ); ?></th-->
						<?php //endif; ?>
						<th class="product-available"><?php esc_html_e( 'AVAILABLE', 'stone' ); ?></th>
						<th class="product-incoming"><?php esc_html_e( 'INCOMING', 'stone' ); ?></th>
						<th class="product-quantity"><?php esc_html_e( 'Quantity', 'stone' ); ?></th>
					</tr>
					</thead>
					<tbody>
					<?php
						...
						if ( ! empty( $raq['yith_wapo_individual_item'] ) && 1 === $raq['yith_wapo_individual_item'] && ! empty( $raq['yith_wapo_parent_key'] ) ) :
							...
							<!--tr class="<?php echo esc_attr( apply_filters( 'yith_ywraq_item_class', 'cart_item', $raq_content, $key ) ); ?>"
								...
								<td class="product-name"
									data-title="<?php esc_attr_e( 'Product', 'stone' ); ?>">
									<?php
									// Meta data.
									$item_data = array();
								...
								<td class="product-uom" data-title="<?php esc_attr_e( 'UOM', 'stone' ); ?>">
									<span>
										<?php
								...
								<?php if ( $show_single_price ) : ?>
									<td class="product-price"
										data-title="<?php esc_attr_e( 'Price', 'stone' ); ?>">
										<?php
							...
							<tr class="<?php echo esc_attr( apply_filters( 'yith_ywraq_item_class', 'cart_item', $raq_content, $key ) ); ?>" <?php echo esc_attr( apply_filters( 'yith_ywraq_item_attributes', '', $raq_content, $key ) ); ?>>
								...
								<td class="product-uom" data-title="UOM">
									<span>
										<?php
										$product_uoms = wp_get_post_terms($product_id, 'product_uom');
										if($product_uoms) {
								...
								<?php //if($is_allow_role) : ?>
									<!--td class="product-price" data-title="<?php //esc_attr_e( 'Price', 'yith-woocommerce-request-a-quote' ); ?>">
										<?php //echo wp_kses_post( apply_filters( 'yith_ywraq_hide_price_template', $price, $product_id, $raq ) ); ?>
										<span>
											<?php 
											//if($_product->get_price()) echo $_product->get_price_html();
											//else echo '&mdash;';
											?>
										</span>
									</td-->
								...
								<td class="product-available" data-title="AVAILABLE">
									<span>
								...
								<td class="product-incoming" data-title="INCOMING">
									<span>
								...
								<?php if ( $show_quantity ) : ?>
									<td class="product-quantity" data-title="<?php esc_attr_e( 'Quantity', 'yith-woocommerce-request-a-quote' ); ?>">
										<div class="qty-wrapper input-wrap">
											<div class="minus qty-btn">
												<svg width="18" height="19" viewBox="0 0 18 19" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M3.57031 10.3613H8.25781H9.74315H14.4307V8.87598H9.74315H8.25781H3.57031V10.3613Z" fill="black"/></svg>
											</div>
											...
											<div class="plus qty-btn">
												<svg width="19" height="19" viewBox="0 0 19 19" fill="none" xmlns="http://www.w3.org/2000/svg"><path d="M8.75781 15.0488V10.3613H4.07031V8.87598H8.75781V4.18848H10.2432V8.87598H14.9307V10.3613H10.2432V15.0488H8.75781Z" fill="black"/></svg>
											</div>
										</div>
									</td>
								<?php endif; ?>
					...
					<?php if ( $show_totals ) : ?>
						...
						if ( $total_tax > 0 && 'incl' !== $tax_display_list && ywraq_show_taxes_on_quote_list() ) :
							...
							<tr>
								...
								<td class="raq-totals" data-title="<?php echo esc_html( WC()->countries->tax_or_vat() ); ?>">
						...
						<tr>
							...
							<th class="raq-totals-label">
								<?php echo esc_html( apply_filters( 'yith_ywraq_quote_table_total_label', __( 'Total:', 'yith-woocommerce-request-a-quote' ) ) ); ?>
							</th>
							<td class="raq-totals"
